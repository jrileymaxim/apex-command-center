  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
      ...
      <Panel label={"◈ EOD BRIEFING"} right={<button className="btn bsm" onClick={onEod}>GENERATE</button>}>
        {!eod&&<div style={{fontSize:"10px",color:CD}}>Reads live prices — hit Refresh on Portfolio first, then GENERATE.</div>}
        {eod&&<div className="fi">{eod.split("\n").map(function(line,li){return li===0?<div key={li} style={{fontFamily:"Orbitron",fontSize:"9px",color:CA,marginBottom:"8px"}}>{line}</div>:line===""?<div key={li} style={{height:"4px"}}/>:<div key={li} style={{fontSize:"11px",color:CB,lineHeight:1.8}}>{line}</div>;})}<div style={{marginTop:"8px",fontSize:"9px",color:CD}}>— MARCUS</div></div>}
      </Panel>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
        ...
        <Panel label={"◈ PARLAYS SUMMARY"}>
          ...
          <div style={{marginTop:"8px",fontSize:"9px",color:CD,letterSpacing:"1px"}}>AT RISK: $112 ▪ MAX PAYOUT: $13,008.93 ▪ SETTLES OCT 31</div>
            ...
            <Panel label={"◈ PREDICTION LOG — SELF LEARNING"}>
        ...
        {predLog.slice(0,8).map(function(pred,pi){
          ...
          return(
            <div key={pred.id} style={{padding:"8px",marginBottom:"4px",background:BD,border:"1px solid #1a1520"}}>
              ...
              <div style={{fontSize:"9px",color:CD,marginTop:"3px"}}>${pred.price.toFixed(2)} {pred.note?"— "+pred.note.slice(0,40):""} {pred.date}</div>
            </div>
          );
        })}
      </Panel>
