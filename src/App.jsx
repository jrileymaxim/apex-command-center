      <Panel label={"â MARCUS PREDICTION LOG â SELF-LEARNING"}>
        <div style={{marginBottom:"10px"}}>
          <div style={{fontSize:"9px",color:CD,letterSpacing:"1px",marginBottom:"6px"}}>LOG A PREDICTION â MARCUS TRACKS ACCURACY</div>
          <div style={{display:"flex",gap:"6px",marginBottom:"6px"}}>
            <select className="inp" value={predTicker} onChange={function(e){setPredTicker(e.target.value);}} style={{flex:1}}>
              <option value="">TICKER...</option>
              {POSITIONS.map(function(p){return <option key={p.ticker} value={p.ticker}>{p.ticker}</option>;})}
            </select>
            <select className="inp" value={predCall} onChange={function(e){setPredCall(e.target.value);}} style={{width:"90px"}}>
          ...
          <div style={{display:"flex",gap:"6px",marginBottom:"6px"}}>
            <input className="inp" style={{flex:1}} value={predTarget} onChange={function(e){setPredTarget(e.target.value);}} placeholder="TARGET PRICE ($)"/>
            <input className="inp" style={{flex:2}} value={predNote} onChange={function(e){setPredNote(e.target.value);}} placeholder="REASONING..."/>
          </div>
          <button className="btn" style={{width:"100%"}} onClick={function(){
            if(!predTicker) return;
            var pos = enriched.find(function(p){return p.ticker===predTicker;});
            var entry = {id:Date.now(),date:new Date().toISOString().slice(0,10),ticker:predTicker,call:predCall,entryPrice:pos?parseFloat(pos.price||pos.avgCost):0,targetPrice:parseFloat(predTarget)||0,notes:predNote,outcome:"OPEN"};
            var updated = [entry].concat(predLog);
            setPredLog(updated);
            localStorage.setItem("apex-predictions",JSON.stringify(updated));
            setPredTicker(""); setPredTarget(""); setPredNote("");
          }}>LOG PREDICTION</button>
        </div>
        {predLog.length>0&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:"6px"}}>
              <div style={{fontSize:"9px",color:CD,letterSpacing:"1px"}}>ACCURACY: {(predLog.filter(function(p){return p.outcome==="WIN";}).length/(Math.max(1,predLog.filter(function(p){return p.outcome!=="OPEN";}).length))*100).toFixed(0)}% ({predLog.filter(function(p){return p.outcome==="WIN";}).length}W / {predLog.filter(function(p){return p.outcome==="LOSS";}).length}L / {predLog.filter(function(p){return p.outcome==="OPEN";}).length} OPEN)</div>
            </div>
            {predLog.slice(0,10).map(function(pred,pi){
              var outcomeCol = pred.outcome==="WIN"?CG:pred.outcome==="LOSS"?CR:CY;
              var pos = enriched.find(function(p){return p.ticker===pred.ticker;});
              var curPrice = pos?parseFloat(pos.price||pos.avgCost):null;
              var pnlPct = curPrice&&pred.entryPrice?((curPrice-pred.entryPrice)/pred.entryPrice*100).toFixed(1):null;
              return(
                <div key={pred.id} style={{padding:"8px",marginBottom:"4px",background:BD,border:"1px solid #1a1520"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"3px"}}>
                    <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                      <span style={{fontFamily:"Orbitron",fontSize:"12px",fontWeight:800,color:CA}}>{pred.ticker}</span>
                      <span style={{fontFamily:"Orbitron",fontSize:"9px",padding:"1px 6px",background:pred.call==="BUY"?CG+"22":pred.call==="SELL"?CR+"22":CY+"22",color:pred.call==="BUY"?CG:pred.call==="SELL"?CR:CY,border:"1px solid "+(pred.call==="BUY"?CG:pred.call==="SELL"?CR:CY)+"44"}}>{pred.call}</span>
                      <span style={{fontFamily:"Orbitron",fontSize:"8px",padding:"1px 5px",background:outcomeCol+"22",color:outcomeCol,border:"1px solid "+outcomeCol+"44"}}>{pred.outcome}</span>
                    </div>
                    <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                      {pnlPct&&<span style={{fontFamily:"Orbitron",fontSize:"10px",color:parseFloat(pnlPct)>0?CG:CR}}>{parseFloat(pnlPct)>0?"+":""}{pnlPct}%</span>}
                      <button style={{background:"transparent",border:"none",color:CD,cursor:"pointer",fontSize:"11px"}} onClick={function(){
                        var updated=predLog.map(function(p){return p.id===pred.id?Object.assign({},p,{outcome:p.outcome==="WIN"?"LOSS":p.outcome==="LOSS"?"OPEN":"WIN"}):p;});
                        setPredLog(updated); localStorage.setItem("apex-predictions",JSON.stringify(updated));
                      }}>â»</button>
                      <button style={{background:"transparent",border:"none",color:CD,cursor:"pointer",fontSize:"11px"}} onClick={function(){
                        var updated=predLog.filter(function(p){return p.id!==pred.id;});
                        setPredLog(updated); localStorage.setItem("apex-predictions",JSON.stringify(updated));
                      }}>Ã</button>
                    </div>
                  </div>
                  <div style={{fontSize:"9px",color:CD}}>Entry: ${pred.entryPrice.toFixed(2)} {pred.targetPrice>0?"â Target: $"+pred.targetPrice.toFixed(2):""} {pred.notes&&"â¸ "+pred.notes.slice(0,50)} <span style={{color:CC}}>{pred.date}</span></div>
                </div>
              );
            })}
          </div>
        )}
        {predLog.length===0&&<div style={{fontSize:"10px",color:CD}}>No predictions logged yet. Log your first call above â Marcus will track whether you were right.</div>}
      </Panel>
import { useState, useEffect } from "react";

// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ PORTFOLIO DATA ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
const POSITIONS = [
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ PHASE 4 DATA ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
const EARNINGS = [
  ...
  {ticker:"TSM", date:"2026-04-17", est:"Q1 2026", note:"Already reported ÃÂ¢ÃÂÃÂ strong AI demand"},
  ...
  {ticker:"CRWV",date:"2026-05-12", est:"Q1 2026", note:"First earnings as public ÃÂ¢ÃÂÃÂ revenue trajectory"},
  ...
  {ticker:"VTI", date:null, est:"ETF", note:"No earnings ÃÂ¢ÃÂÃÂ tracks total market"},
  {ticker:"GLD", date:null, est:"ETF", note:"No earnings ÃÂ¢ÃÂÃÂ tracks gold price"},
];
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ COLORS ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
const CA="#f0a30a",CB="#ffffff",CC="#d4a84b",CD="#7a6030";
const CG="#18c93a",CR="#e03010",CY="#e8b820";
const BG="#030308",BP="#0a0a18",BD="#0d0d22";

// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ PHASE 2 DATA ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ

// Signal engine ÃÂ¢ÃÂÃÂ derives buy/hold/sell from price vs cost basis + day change
...
const SOUN_DATA = {
  ...
  catalysts: [
    {date:"Q2 2025",event:"SoundHound AI automotive partnership expansion ÃÂ¢ÃÂÃÂ 100+ car brands"},
    {date:"Q3 2025",event:"Restaurant industry rollout ÃÂ¢ÃÂÃÂ 10,000+ locations active"},
    ...
    {date:"Apr 2026",event:"Earnings report upcoming ÃÂ¢ÃÂÃÂ analyst consensus: revenue growth 70%+ YoY"},
  ],
  sentiment: [
    {label:"Reddit/StockTwits",score:72,color:CG,note:"Bullish ÃÂ¢ÃÂÃÂ high retail interest"},
    {label:"Analyst Consensus",score:58,color:CY,note:"Mixed ÃÂ¢ÃÂÃÂ 3 Buy, 2 Hold, 1 Sell"},
    {label:"News Tone",score:65,color:CG,note:"Positive ÃÂ¢ÃÂÃÂ AI tailwinds narrative"},
    {label:"Options Flow",score:70,color:CG,note:"Call-heavy ÃÂ¢ÃÂÃÂ bullish positioning"},
  ],
  risks: [
    "Revenue growth not yet profitable ÃÂ¢ÃÂÃÂ cash burn risk",
    "Highly competitive AI voice space (Google, Amazon, Apple)",
    "Small-cap volatility ÃÂ¢ÃÂÃÂ high beta stock",
    "Breakeven requires +60% to +76% move from current price",
  ],
  bull: "SOUN is positioned at the intersection of AI and voice commerce ÃÂ¢ÃÂÃÂ two of the fastest growing verticals. Automotive and restaurant deployments provide recurring SaaS revenue, and the Amelia acquisition adds enterprise AI agent capability. If AI infrastructure spending accelerates in 2025-2026, SOUN's platform becomes mission-critical.",
  bear: "The company remains unprofitable with significant cash burn. Voice AI is becoming commoditized by Big Tech. SOUN's $11-12 breakeven prices require a near-double from current levels ÃÂ¢ÃÂÃÂ a high bar for a company still proving its business model.",
};
...
// Discovery picks ÃÂ¢ÃÂÃÂ curated high-conviction AI/tech small caps
...
function marcusBriefing() {
  ...
  const closer = h>=20?"Rest when the work is done ÃÂ¢ÃÂÃÂ but the work is not done.":"";
  const q = [
    "The disciplined mind does not panic at volatility ÃÂ¢ÃÂÃÂ it studies it. Markets reveal character.",
    "Wealth is not built in moments of excitement. It is built in moments of discipline.",
    "The obstacle is the way. Volatility is not the enemy ÃÂ¢ÃÂÃÂ indecision is.",
    ...
    "Eleven positions, two LEAPS, four parlays. Diversification is not weakness ÃÂ¢ÃÂÃÂ it is strategy.",
    "The early review catches the edge. Your positions await analysis.",
  ];
  return g+". Today is "+day+", "+date+". Eleven positions, two SOUN LEAPS, four MLB futures settling October 31st ÃÂ¢ÃÂÃÂ all require your attention. "+q[now.getDate()%q.length]+(closer?" "+closer:"");
}
...
async function fetchPrices() {
  ...
  try {
    ...
    console.warn("fetchPrices failed, using fallback:", e.message);
    return FALLBACK;
  }
}
...
async function fetchWeather(city) {
  ...
  return {
    temp:    Math.round(c.temperature_2m)+"ÃÂÃÂ°F",
    feels:   Math.round(c.apparent_temperature)+"ÃÂÃÂ°F",
    cond:    cond,
    humidity:c.relativehumidity_2m+"%",
    wind:    Math.round(c.windspeed_10m)+" mph",
    high:    Math.round(wd.daily.temperature_2m_max[0])+"ÃÂÃÂ°F",
    low:     Math.round(wd.daily.temperature_2m_min[0])+"ÃÂÃÂ°F",
    city:    loc.name,
  };
}
...
async function fetchBenchmarks() {
  try {
    ...
    var out = {};
    results.forEach(function(q){
      out[q.symbol]={price:q.regularMarketPrice.toFixed(2),chg:q.regularMarketChangePercent.toFixed(2)};
    });
    return out;
  } catch(e) {
    return {SPY:{price:"--",chg:"0"},QQQ:{price:"--",chg:"0"},DIA:{price:"--",chg:"0"}};
  }
}
...
function weeklyReport(enriched, totP, fg) {
  ...
  return prefix+"Portfolio return: "+ret+". Sentiment: "+(fg&&fg.label||"Neutral")+". "+topStr+". "+worstStr+". "+(isMonday?"New week ÃÂ¢ÃÂÃÂ set objectives and review stop-losses.":"Stay disciplined.");
}

// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ MONTE CARLO ENGINE ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ STYLES ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
function generateEOD(enriched,totCost,totMkt,totGain,totP,bench,fg){
  if(!enriched||!enriched.length) return "Load portfolio data first.";
  var now=new Date();
  ...
  var byDay=wd.slice().sort(function(a,b){return b.chgP-a.chgP;});
  var topPos=byDay[0], botPos=byDay[byDay.length-1];
  var upCt=wd.filter(function(p){return p.chgP>0;}).length;
  var dnCt=wd.filter(function(p){return p.chgP<0;}).length;
  var dayDollar=wd.reduce(function(acc,p){return acc+(p.mkt!=null&&p.chgP!=null?(p.mkt-p.mkt/(1+p.chgP/100)):0);},0);
  var ai=["NVDA","TSM","CRWV","SMCI","ANET","MU"];
  var aiUp=enriched.filter(function(p){return ai.indexOf(p.ticker)>-1&&p.chgP>0;}).length;
  var aiDn=enriched.filter(function(p){return ai.indexOf(p.ticker)>-1&&p.chgP<0;}).length;
  var bd=bench&&bench.data||{};
  var lines=[
    "APEX EOD ÃÂ¢ÃÂÃÂ "+ds+" at "+ts,
    "",
    "Portfolio: $"+totMkt.toFixed(2)+" / $"+totCost.toFixed(2)+" cost. Return: "+(totGain>=0?"+":"")+totP.toFixed(2)+"% ($"+(totGain>=0?"+":"")+totGain.toFixed(2)+").",
    upCt+" up, "+dnCt+" down today. Est. day P&L: "+(dayDollar>=0?"+":"")+dayDollar.toFixed(2)+".",
    "",
    topPos?"Best today: "+topPos.ticker+" "+(topPos.chgP>0?"+":"")+topPos.chgP.toFixed(2)+"% at $"+topPos.price+".":"",
    botPos&&botPos.chgP<0?"Worst today: "+botPos.ticker+" "+botPos.chgP.toFixed(2)+"% at $"+botPos.price+".":"",
    "",
    "AI cluster (NVDA/TSM/CRWV/SMCI/ANET/MU): "+aiUp+" up, "+aiDn+" down.",
    bd.SPY?"Market: SPY "+bd.SPY.chg+"% QQQ "+(bd.QQQ?bd.QQQ.chg+"%":"--"):"",
    "",
    "Sentiment: "+(fg&&fg.label||"Neutral")+". Stay disciplined.",
  ];
  return lines.filter(function(l,li){return li===0||l.length>0;}).join("\n");
}
...
.xbtn{background:transparent;color:#7a6030;border:1px solid #7a603044;padding:2px 10px;cursor:pointer;font-family:'Share Tech Mono',monospace;font-size:16px;line-height:1;}
.xbtn:hover{color:#ffffff;border-color:#ffffff44;}
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ MAIN APP ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
export default function App() {
    ...
    (async function(){
      try {
        ...
        if (_t) setTasks(JSON.parse(_t));
          ...
          {id:1,text:"Review MNTS ÃÂ¢ÃÂÃÂ stop-loss evaluation",done:false},
          {id:2,text:"Monitor DVN ÃÂ¢ÃÂÃÂ analyst downgrade risk",done:false},
  ...
  var TABS=[
    {id:"briefing",icon:"ÃÂ¢ÃÂÃÂ¡",l:"BRIEFING"},
    {id:"portfolio",icon:"ÃÂ°ÃÂÃÂÃÂ",l:"PORTFOLIO"},
    {id:"intel",icon:"ÃÂ°ÃÂÃÂÃÂ",l:"INTEL"},
    {id:"soun",icon:"ÃÂ°ÃÂÃÂÃÂ¯",l:"SOUN OPS"},
    {id:"parlays",icon:"ÃÂ¢ÃÂÃÂ¾",l:"PARLAYS"},
    {id:"alerts",icon:"ÃÂ°ÃÂÃÂÃÂ",l:"ALERTS"},
    {id:"mission",icon:"ÃÂ°ÃÂÃÂÃÂ ",l:"MISSION"},
  ];
  ...
  return (
    <div style={{height:"100vh",overflow:"hidden",background:BG,color:CC,fontFamily:"'Share Tech Mono',monospace",display:"flex",flexDirection:"column",position:"relative"}} className={alertOn?"alertMode":""}>
      ...
      <div style={{position:"relative",zIndex:10,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 16px",background:alertOn?"linear-gradient(#150005,#08000a)":"linear-gradient(#0d0a04,#050510)",borderBottom:"1px solid "+(alertOn?"#3d0000":"#1a1408"),transition:"all .5s"}}>
        <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
          ...
          <div>
            ...
            <div style={{fontSize:"9px",letterSpacing:"2px",color:CD,marginTop:"2px"}}>{alertOn?"ÃÂ¢ÃÂÃÂ  ALERT ÃÂ¢ÃÂÃÂ POSITION MOVEMENT DETECTED":"PORTFOLIO INTELLIGENCE ÃÂ¢ÃÂÃÂª V2.0 ÃÂ¢ÃÂÃÂª PHASE 1"}</div>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
          <button className="btn bsm" onClick={function(){setAlert(function(v){return !v;});}} style={{color:alertOn?"#ff4422":CA,borderColor:alertOn?"#3d0000":"#2a1e08"}}>{alertOn?"ÃÂ¢ÃÂÃÂ ALERT":"ÃÂ¢ÃÂÃÂ ALERT"}</button>
          <button className="btn bsm" onClick={function(){setVoice(function(v){return !v;});}} style={{color:voice?"#1a7acc":CA,borderColor:voice?"#0a3050":"#2a1e08"}}>{voice?"ÃÂ°ÃÂÃÂÃÂ ON":"ÃÂ°ÃÂÃÂÃÂ OFF"}</button>
          <button className="btn bsm" onClick={fireConfetti}>ÃÂ°ÃÂÃÂÃÂ</button>
      ...
      <div style={{flex:1,minHeight:0,overflow:"auto",position:"relative",zIndex:1,padding:"12px 14px"}}>
        ...
        {tab==="alerts"    && <TabAlerts filings={edgarFilings} loading={edgarLoading} enabled={alertsEnabled} onEnable={async function(){if("Notification" in window){var p=await Notification.requestPermission();setAlertsEnabled(p==="granted");}}} onScan={async function(){setEdgarLoading(true);var f=await fetchEdgarFilings();setEdgarFilings(f);setEdgarLoading(false);if(alertsEnabled)f.filter(function(x){return x.urgent;}).slice(0,3).forEach(function(x){new Notification("APEX ÃÂ¢ÃÂÃÂ "+x.ticker+" "+x.form,{body:x.date});});}}/>}
      ...
      <div style={{position:"relative",zIndex:10,flexShrink:0,display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 16px",background:"#020208",borderTop:"1px solid #14100a"}}>
        <div style={{fontSize:"9px",letterSpacing:"1.5px",color:CD}}>APEX V2.0 ÃÂ¢ÃÂÃÂª {POSITIONS.length} POSITIONS ÃÂ¢ÃÂÃÂª 2 LEAPS ÃÂ¢ÃÂÃÂª 4 PARLAYS ÃÂ¢ÃÂÃÂª PHASE 1</div>
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ BRIEFING TAB ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
function TabBriefing({alf,onAlf,wx,city,setCity,onWx,fg,enriched,totP,eod,onEod}) {
  ...
  function wIcon(c) {
    ...
    if(cl.includes("thunder"))return"ÃÂ¢ÃÂÃÂ";
    if(cl.includes("rain")||cl.includes("shower"))return"ÃÂ°ÃÂÃÂÃÂ§";
    if(cl.includes("snow"))return"ÃÂ¢ÃÂÃÂ";
    if(cl.includes("fog"))return"ÃÂ°ÃÂÃÂÃÂ«";
    if(cl.includes("cloud")||cl.includes("overcast"))return"ÃÂ¢ÃÂÃÂ";
    if(cl.includes("clear")||cl.includes("sun")||cl.includes("sky"))return"ÃÂ¢ÃÂÃÂ";
    return"ÃÂ¢ÃÂÃÂ";
  }
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
      <Panel label={"ÃÂ¢ÃÂÃÂ MARCUS ÃÂ¢ÃÂÃÂ MORNING BRIEFING"} right={<button className="btn bsm" onClick={onAlf}>ÃÂ¢ÃÂÃÂ»</button>}>
        {alf.s==="done"&&(
          <div className="fi">
            <p style={{fontSize:"12px",lineHeight:1.9,color:CB,fontStyle:"italic",marginBottom:"12px"}}>"{alf.text}"</p>
            <div style={{fontSize:"9px",letterSpacing:"2px",color:CD}}>ÃÂ¢ÃÂÃÂ MARCUS ÃÂ¢ÃÂÃÂª APEX</div>
          </div>
        )}
      </Panel>

      <Panel label={"ÃÂ¢ÃÂÃÂ MARKET SENTIMENT"}>
        {fg.s==="done"&&(
      ...
      <Panel label={"ÃÂ¢ÃÂÃÂ ATMOSPHERIC CONDITIONS"}>
        ...
        {wx.s==="idle"&&<div style={{fontSize:"10px",color:CD}}>ÃÂ¢ÃÂÃÂ¸ Enter city to scan conditions</div>}
      ...
      <Panel label={isMonday?"ÃÂ¢ÃÂÃÂ MARCUS ÃÂ¢ÃÂÃÂ WEEKLY REPORT":"ÃÂ¢ÃÂÃÂ MARCUS ÃÂ¢ÃÂÃÂ PORTFOLIO SNAPSHOT"}>
        <div style={{fontSize:"11px",color:CB,lineHeight:1.8,fontStyle:"italic",marginBottom:"8px"}}>"{report}"</div>
        <div style={{fontSize:"9px",color:CD,letterSpacing:"2px"}}>ÃÂ¢ÃÂÃÂ MARCUS {isMonday?"ÃÂ¢ÃÂÃÂª MONDAY REVIEW":""}</div>
      </Panel>

      <Panel label={"ÃÂ¢ÃÂÃÂ MARCUS ÃÂ¢ÃÂÃÂ END OF DAY BRIEFING"} right={<button className="btn bsm" onClick={onEod}>GENERATE</button>}>
        {!eod&&<div style={{fontSize:"10px",color:CD,lineHeight:1.8}}>Reads your live portfolio prices ÃÂ¢ÃÂÃÂ real data, no web search. Refresh Portfolio first then hit GENERATE.</div>}
        {eod&&(
          <div className="fi">
            {eod.split("\n").map(function(line,li){
              if(li===0) return <div key={li} style={{fontFamily:"Orbitron",fontSize:"9px",color:CA,letterSpacing:"1px",marginBottom:"10px"}}>{line}</div>;
              if(line==="") return <div key={li} style={{height:"5px"}}/>;
              return <div key={li} style={{fontSize:"11px",color:CB,lineHeight:1.8}}>{line}</div>;
            })}
            <div style={{marginTop:"10px",fontSize:"9px",color:CD,letterSpacing:"2px"}}>ÃÂ¢ÃÂÃÂ MARCUS ÃÂ¢ÃÂÃÂª LIVE DATA</div>
          </div>
        )}
      </Panel>

      <Panel label={"ÃÂ¢ÃÂÃÂ SYSTEM STATUS"}>
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ PORTFOLIO TAB ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
function TabPortfolio({enriched,totCost,totMkt,totGain,totP,leaps,status,live,onRefresh,bench}) {
  ...
  var [tgtAmt,setTgtAmt]=useState("");
  var [tgtDays,setTgtDays]=useState("");
  var [tgtResult,setTgtResult]=useState(null);
  var bd=bench&&bench.data||{};
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      ...
      <Panel label={"ÃÂ¢ÃÂÃÂ POSITIONS ÃÂ¢ÃÂÃÂ "+POSITIONS.length+" HOLDINGS "+(live?"ÃÂ¢ÃÂÃÂª LIVE":"ÃÂ¢ÃÂÃÂª SESSION DATA 4/14")} right={<button className="btn bsm" onClick={onRefresh} disabled={status==="loading"}>ÃÂ¢ÃÂÃÂ» REFRESH</button>}>
        ...
        {status!=="loading"&&enriched.map(function(p,i){return (
          <div key={p.ticker} className="pr si" style={{animationDelay:i*30+"ms"}}>
            ...
            <Panel label={"ÃÂ¢ÃÂÃÂ PORTFOLIO TARGET CALCULATOR"}>
        <div style={{display:"flex",gap:"8px",marginBottom:"10px"}}>
          <div style={{flex:1}}>
            <div style={{fontSize:"9px",color:CD,marginBottom:"4px",letterSpacing:"1px"}}>GOAL ($)</div>
            <input className="inp" value={tgtAmt} onChange={function(e){setTgtAmt(e.target.value);setTgtResult(null);}} placeholder="50000"/>
          </div>
          <div style={{flex:1}}>
            <div style={{fontSize:"9px",color:CD,marginBottom:"4px",letterSpacing:"1px"}}>DAYS</div>
            <input className="inp" value={tgtDays} onChange={function(e){setTgtDays(e.target.value);setTgtResult(null);}} placeholder="365"/>
          </div>
        </div>
        <button className="btn" style={{width:"100%",marginBottom:"10px"}} onClick={function(){
          var goal=parseFloat(tgtAmt),days=parseInt(tgtDays);
          if(!goal||!days) return;
          var cur=enriched.reduce(function(a,px){return a+(px.mkt||px.avgCost*px.shares);},0);
          var need=goal-cur;
          if(need<=0){setTgtResult({msg:"Already at goal! Current: $"+cur.toFixed(2),buys:[]});return;}
          var ranked=enriched.filter(function(px){return px.chgP!=null;}).slice().sort(function(a,bx){return (bx.chgP||0)-(a.chgP||0);});
          var buys=ranked.slice(0,4).map(function(px){
            var dollar=need*0.25, shs=dollar/(parseFloat(px.price)||px.avgCost);
            return {t:px.ticker,p:(parseFloat(px.price)||px.avgCost).toFixed(2),d:dollar.toFixed(2),s:shs.toFixed(3)};
          });
          setTgtResult({msg:"Need $"+need.toFixed(2)+" over "+days+" days (~$"+(need/days).toFixed(2)+"/day):",buys:buys});
        }}>CALCULATE</button>
        {tgtResult&&(
          <div className="fi">
            <div style={{fontSize:"10px",color:CA,marginBottom:"8px"}}>{tgtResult.msg}</div>
            {tgtResult.buys.map(function(bx,bi){
              return(<div key={bi} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #0c0a14"}}><span style={{fontFamily:"Orbitron",fontSize:"11px",color:CA}}>{bx.t}</span><span style={{fontSize:"10px",color:CB}}>{bx.s} sh @ ${bx.p}</span><span style={{fontFamily:"Orbitron",fontSize:"10px",color:CG}}>${bx.d}</span></div>);
            })}
          </div>
        )}
      </Panel>
<Panel label={"ÃÂ¢ÃÂÃÂ SOUN LEAPS ÃÂ¢ÃÂÃÂ OPTIONS POSITIONS"}>
        ...
        <div style={{marginTop:"8px",fontSize:"9px",color:CD,letterSpacing:"1px"}}>ÃÂ¢ÃÂÃÂ¸ Full Greeks, theta clock & probability ÃÂ¢ÃÂÃÂ SOUN OPS tab</div>
      </Panel>

      <Panel label={"ÃÂ¢ÃÂÃÂ EARNINGS CALENDAR ÃÂ¢ÃÂÃÂ NEXT 60 DAYS"}>
      ...
      <Panel label={"ÃÂ¢ÃÂÃÂ PERFORMANCE VS BENCHMARKS"}>
...
async function fetchEdgarFilings() {
  ...
  var all = [];
  await Promise.all(Object.entries(CIKS).map(async function(entry){
    var ticker=entry[0], cik=entry[1];
    try {
      var r = await fetch("https://data.sec.gov/submissions/CIK"+cik+".json",{headers:{"User-Agent":"apex/1.0 monitor@apex.app"}});
      var d = await r.json();
      var recent = d.filings&&d.filings.recent||{};
      var forms=recent.form||[], dates=recent.filingDate||[], docs=recent.primaryDocument||[];
      for(var i=0;i<Math.min(forms.length,10);i++){
        if(dates[i]>=cutoff && ["8-K","4","SC 13D","SC 13G","10-Q","10-K"].indexOf(forms[i])>-1){
          all.push({ticker:ticker,form:forms[i],date:dates[i],doc:docs[i]||"",urgent:forms[i]==="8-K"||forms[i]==="4",url:"https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK="+cik+"&type="+forms[i]+"&dateb=&owner=include&count=5"});
        }
      }
    } catch(e) {}
  }));
...
function TabAlerts({filings,loading,enabled,onEnable,onScan}){
  var urgent=filings.filter(function(f){return f.urgent;});
  var normal=filings.filter(function(f){return !f.urgent;});
  var formColor=function(form){return form==="8-K"||form==="4"?CR:form==="SC 13D"?CY:CG;};
  var formLabel=function(form){var m={"8-K":"MATERIAL EVENT","4":"INSIDER TRADE","10-Q":"QUARTERLY","10-K":"ANNUAL","SC 13D":"ACTIVIST","SC 13G":"LARGE HOLDER"};return m[form]||form;};
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      <Panel label={"ÃÂ¢ÃÂÃÂ EDGAR + ALERTS CONTROL"}>
        <div style={{display:"flex",gap:"8px",marginBottom:"10px"}}>
          <button className="btn" style={{flex:1}} onClick={onScan}>{loading?"SCANNING...":"ÃÂ¢ÃÂÃÂ» SCAN SEC EDGAR"}</button>
          <button className="btn" style={{flex:1,color:enabled?CG:CA,borderColor:enabled?CG:CA}} onClick={onEnable}>{enabled?"ÃÂ°ÃÂÃÂÃÂ ALERTS ON":"ÃÂ°ÃÂÃÂÃÂ ENABLE ALERTS"}</button>
        </div>
        <div style={{fontSize:"9px",color:CD,lineHeight:1.7}}>Scans SEC EDGAR for 8-K material events, Form 4 insider trades, SC 13D activist filings, and earnings reports for all your positions. Browser push notifications fire on urgent filings. 24/7 GitHub Actions monitor also runs every 5 min and sends ntfy alerts to your phone.</div>
      </Panel>
      {urgent.length>0&&(
        <Panel label={"ÃÂ¢ÃÂÃÂ URGENT ÃÂ¢ÃÂÃÂ "+urgent.length+" FILING"+(urgent.length!==1?"S":"")}>
          {urgent.map(function(f,fi){
            return(
              <div key={fi} style={{padding:"10px",marginBottom:"6px",background:"rgba(224,48,16,.06)",border:"1px solid "+CR+"44"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"4px"}}>
                  <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                    <span style={{fontFamily:"Orbitron",fontSize:"12px",fontWeight:800,color:CA}}>{f.ticker}</span>
                    <span style={{fontFamily:"Orbitron",fontSize:"8px",padding:"2px 6px",background:formColor(f.form)+"22",color:formColor(f.form),border:"1px solid "+formColor(f.form)+"44"}}>{formLabel(f.form)}</span>
                    <span style={{fontFamily:"Orbitron",fontSize:"8px",color:CR}}>ÃÂ¢ÃÂÃÂ  URGENT</span>
                  </div>
                  <span style={{fontSize:"9px",color:CD}}>{f.date}</span>
                </div>
                <a href={f.url} target="_blank" rel="noreferrer" style={{fontSize:"9px",color:CA,textDecoration:"none",letterSpacing:"1px"}}>ÃÂ¢ÃÂÃÂ¸ VIEW ON SEC.GOV ÃÂ¢ÃÂÃÂ</a>
              </div>
            );
          })}
        </Panel>
      )}
      <Panel label={"ÃÂ¢ÃÂÃÂ SEC EDGAR ÃÂ¢ÃÂÃÂ LAST 7 DAYS"+(filings.length>0?" ("+filings.length+")":"")}>
        {loading&&<Spinner label="SCANNING EDGAR..."/>}
        {!loading&&filings.length===0&&<div style={{fontSize:"10px",color:CD}}>Hit SCAN to fetch latest filings for all positions.</div>}
        {!loading&&normal.map(function(f,fi){
          return(
            <div key={fi} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:"1px solid #0c0a14"}}>
              <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                <span style={{fontFamily:"Orbitron",fontSize:"11px",fontWeight:700,color:CA,minWidth:"42px"}}>{f.ticker}</span>
                <span style={{fontFamily:"Orbitron",fontSize:"8px",padding:"1px 5px",background:formColor(f.form)+"22",color:formColor(f.form),border:"1px solid "+formColor(f.form)+"33"}}>{formLabel(f.form)}</span>
              </div>
              <div style={{display:"flex",gap:"10px",alignItems:"center"}}>
                <span style={{fontSize:"9px",color:CD}}>{f.date}</span>
                <a href={f.url} target="_blank" rel="noreferrer" style={{fontSize:"9px",color:CC,textDecoration:"none"}}>VIEW ÃÂ¢ÃÂÃÂ</a>
              </div>
            </div>
          );
        })}
      </Panel>
      <Panel label={"ÃÂ¢ÃÂÃÂ UNUSUAL WHALES ÃÂ¢ÃÂÃÂ OPTIONS FLOW"}>
        <div style={{fontSize:"10px",color:CD,lineHeight:1.7,marginBottom:"10px"}}>Options flow shows when large money buys calls/puts before a move. Full per-ticker flow requires an Unusual Whales API key (~$50/month).</div>
        <div style={{padding:"10px",background:BD,border:"1px solid #1a1520"}}>
          <div style={{fontFamily:"Orbitron",fontSize:"9px",color:CA,letterSpacing:"2px",marginBottom:"6px"}}>ADD API KEY TO UNLOCK</div>
          <input className="inp" style={{width:"100%",marginBottom:"8px"}} placeholder="UNUSUAL WHALES API KEY..."/>
          <button className="btn" style={{width:"100%"}}>CONNECT FLOW</button>
        </div>
      </Panel>
    </div>
  );
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ MISSION TAB ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
function TabMission({tasks,newTask,setNewTask,onAdd,onToggle,onDel,wl,newWl,setNewWl,onAddWl,onDelWl,journal,setJournal,enriched}) {
  var [predLog,setPredLog]=useState(function(){try{var s=localStorage.getItem("apex-predictions");return s?JSON.parse(s):[];}catch(e){return [];}});
  var [predTicker,setPredTicker]=useState("");
  var [predCall,setPredCall]=useState("BUY");
  var [predTarget,setPredTarget]=useState("");
  var [predNote,setPredNote]=useState("");
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
      <Panel label={"ÃÂ¢ÃÂÃÂ MISSION OBJECTIVES"}>
        <div style={{display:"flex",gap:"6px",marginBottom:"10px"}}>
        ...
        <div style={{maxHeight:"260px",overflow:"auto"}}>
          {tasks.map(function(t){return (
            <div key={t.id} style={{display:"flex",alignItems:"flex-start",gap:"8px",padding:"7px 0",borderBottom:"1px solid #0c0a14",opacity:t.done?.3:1,transition:"opacity .3s"}}>
              <button onClick={function(){onToggle(t.id);}} style={{marginTop:"2px",width:"14px",height:"14px",border:"1px solid "+(t.done?CA:"#2e2210"),background:t.done?CA:"transparent",color:BG,fontSize:"9px",cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>{t.done?"ÃÂ¢ÃÂÃÂ":""}</button>
              <span style={{flex:1,fontSize:"11px",color:t.done?CD:CB,textDecoration:t.done?"line-through":"none",lineHeight:1.5}}>{t.text}</span>
              <button className="xbtn" onClick={function(){onDel(t.id);}}>ÃÂÃÂ</button>
            </div>
          );})}
        </div>
        <div style={{marginTop:"10px",fontFamily:"Orbitron",fontSize:"9px",letterSpacing:"2px",color:CD}}>{tasks.length-done} ACTIVE ÃÂ¢ÃÂÃÂª {done} COMPLETE ÃÂ¢ÃÂÃÂª SAVED ÃÂ¢ÃÂÃÂ</div>
      </Panel>

      <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
        <Panel label={"ÃÂ¢ÃÂÃÂ CUSTOM WATCHLIST"}>
          <div style={{display:"flex",gap:"6px",marginBottom:"10px"}}>
          ...
          {!wl.length&&<div style={{fontSize:"10px",color:CD}}>ÃÂ¢ÃÂÃÂ¸ Add tickers to monitor</div>}
          <div style={{display:"flex",flexWrap:"wrap",gap:"6px"}}>
            {wl.map(function(w){return (
              <div key={w.id} style={{display:"flex",alignItems:"center",gap:"4px",padding:"4px 8px",background:BD,border:"1px solid #1a1520"}}>
                ...
                <button className="xbtn" onClick={function(){onDelWl(w.id);}}>ÃÂÃÂ</button>
              </div>
            );})}
          </div>
        </Panel>

        <Panel label={"ÃÂ¢ÃÂÃÂ PARLAYS SUMMARY"}>
          {PARLAYS.map(function(p,i){return (
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:"1px solid #0c0a14"}}>
              <div>
                ...
                <div style={{fontSize:"10px",color:CD,marginTop:"2px"}}>{p.legs} LEGS ÃÂ¢ÃÂÃÂª ${p.stake} STAKE</div>
              </div>
              <div style={{textAlign:"right"}}>
          ...
          <div style={{marginTop:"8px",fontSize:"9px",color:CD,letterSpacing:"1px"}}>AT RISK: $112 ÃÂ¢ÃÂÃÂª MAX PAYOUT: $13,008.93 ÃÂ¢ÃÂÃÂª SETTLES OCT 31</div>
        </Panel>
      </div>

      <Panel label={"ÃÂ¢ÃÂÃÂ TRADE JOURNAL ÃÂ¢ÃÂÃÂ "+((journal&&journal.length)||0)+" ENTRIES"} right={<button className="btn bsm" onClick={function(){setShowJournal(function(v){return !v;});}}>{showJournal?"HIDE":"SHOW"}</button>}>
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ COMING SOON ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px",maxWidth:"480px",width:"100%"}}>
        {items.map(function(it){return (
          <div key={it} style={{padding:"7px 10px",background:BP,border:"1px solid #141020",fontSize:"9px",color:CD,letterSpacing:"1px"}}>
            <span style={{color:CA}}>ÃÂ¢ÃÂÃÂ¸ </span>{it}
          </div>
        );})}
      </div>
    </div>
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ INTELLIGENCE TAB ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
function TabIntel({enriched,status,totP,fg}) {
  var signals = enriched.map(function(p) {
    var chg = parseFloat(p.chgP)||0;
    var gain = p.gainP||0;
    var price = parseFloat(p.price||p.avgCost);
    var cost = p.avgCost;
    // Signal logic: momentum + position vs cost
    var momentum = chg > 2 ? "BULLISH" : chg < -2 ? "BEARISH" : "NEUTRAL";
    var posture = gain > 15 ? "STRONG" : gain > 0 ? "POSITIVE" : gain > -10 ? "CAUTION" : "WEAK";
    var signal = (momentum==="BULLISH"&&posture!=="WEAK") ? "BUY" : (momentum==="BEARISH"||posture==="WEAK") ? "SELL" : "HOLD";
    var confidence = Math.min(95, Math.max(20, Math.abs(chg)*15 + Math.abs(gain)*0.5 + 40));
    return {ticker:p.ticker, signal:signal, confidence:confidence.toFixed(0), momentum:momentum, posture:posture, chg:chg.toFixed(2), gain:gain.toFixed(1), price:price.toFixed(2)};
  });

  var buys = signals.filter(function(s){return s.signal==="BUY";}).sort(function(a,b){return b.confidence-a.confidence;});
  ...
  var sigColor = function(s){return s==="BUY"?CG:s==="SELL"?CR:CY;};

  return (
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      <Panel label={"â POSITION SIGNALS â LIVE DATA DRIVEN"}>
        {status==="loading"&&<Spinner label="LOADING SIGNALS..."/>}
        {status!=="loading"&&signals.map(function(s,si){
          return(
            <div key={si} style={{display:"flex",alignItems:"center",gap:"8px",padding:"7px 0",borderBottom:"1px solid #0c0a14",cursor:"pointer"}} onClick={function(){setDebateTicker(s.ticker);setDebateMode(true);}}>
              <span style={{fontFamily:"Orbitron",fontSize:"12px",fontWeight:800,color:CA,minWidth:"46px"}}>{s.ticker}</span>
              <span style={{fontFamily:"Orbitron",fontSize:"9px",padding:"2px 8px",background:sigColor(s.signal)+"22",color:sigColor(s.signal),border:"1px solid "+sigColor(s.signal)+"44",minWidth:"42px",textAlign:"center"}}>{s.signal}</span>
              <div style={{flex:1,height:"4px",background:"#0c0a14",borderRadius:"2px"}}>
                <div style={{height:"100%",width:s.confidence+"%",background:sigColor(s.signal),borderRadius:"2px"}}/>
              </div>
              <span style={{fontFamily:"Orbitron",fontSize:"9px",color:sigColor(s.signal),minWidth:"36px",textAlign:"right"}}>{s.confidence}%</span>
              <span style={{fontSize:"9px",color:parseFloat(s.chg)>0?CG:CR,minWidth:"50px",textAlign:"right"}}>{parseFloat(s.chg)>0?"+":""}{s.chg}% today</span>
              <span style={{fontSize:"9px",color:parseFloat(s.gain)>0?CG:CR,minWidth:"50px",textAlign:"right"}}>{parseFloat(s.gain)>0?"+":""}{s.gain}% total</span>
            </div>
          );
        })}
        <div style={{marginTop:"8px",fontSize:"9px",color:CD}}>Signals based on live price momentum + total return posture. Tap any ticker for Bull vs Bear debate.</div>
      </Panel>

      <Panel label={"â MARKET CONVICTION SUMMARY"}>
      ...
      {debateMode&&<DebateView ticker={debateTicker} enriched={enriched} onClose={function(){setDebateMode(false);}}/>}
    </div>
  );
}) {
  var [debateMode,setDebateMode] = useState(false);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      ...
      {/* Signals List */}
      <Panel label={"ÃÂ¢ÃÂÃÂ POSITION SIGNALS ÃÂ¢ÃÂÃÂ AI DERIVED"}>
      ...
      {/* Discovery Picks */}
      <Panel label={"ÃÂ¢ÃÂÃÂ MORNING DISCOVERY ÃÂ¢ÃÂÃÂ HIGH CONVICTION PICKS"}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px"}}>
          {DISCOVERY_PICKS.map(function(pick){return (
            <div key={pick.ticker} style={{background:BD,border:"1px solid #1a1520",padding:"10px",position:"relative"}}>
              ...
              <div style={{fontSize:"9px",color:CD,letterSpacing:"2px",marginBottom:"6px"}}>{pick.sector} ÃÂ¢ÃÂÃÂª {pick.timeframe}</div>
              <div style={{fontSize:"12px",color:CB,lineHeight:1.6,marginBottom:"6px"}}>{pick.price} ÃÂ¢ÃÂÃÂ {pick.thesis}</div>
              <div style={{fontSize:"10px",color:CR,lineHeight:1.5}}>ÃÂ¢ÃÂÃÂ  {pick.risk}</div>
            </div>
          );})}
        </div>
      </Panel>
      ...
      {/* Bull vs Bear Debate */}
      <Panel label={"ÃÂ¢ÃÂÃÂ AI BULL vs BEAR DEBATE"}>
        <div style={{display:"flex",gap:"6px",marginBottom:"12px",alignItems:"center"}}>
          ...
          <button className="btn bsm" onClick={function(){setDebateMode(function(v){return !v;});}}>
            {debateMode?"CLOSE DEBATE":"ÃÂ¢ÃÂÃÂ¸ RUN DEBATE"}
          </button>
        </div>
        {debateMode&&<DebateView ticker={debateTicker} enriched={enriched}/>}
        {!debateMode&&<div style={{fontSize:"10px",color:CD}}>ÃÂ¢ÃÂÃÂ¸ Select a ticker and run the AI debate to see bull vs bear case</div>}
      </Panel>
    </div>
    </div>
    </div>
...
function DebateView({ticker,enriched}) {
  ...
  var bullPoints = [
    ...
    "Portfolio cost basis of "+usd(p.avgCost)+" vs current "+usd(p.cur)+" shows "+(p.gainP>=0?"a gain":"manageable loss")+" ÃÂ¢ÃÂÃÂ thesis intact.",
  ];
  var bearPoints = [
    "Macro headwinds ÃÂ¢ÃÂÃÂ Fed policy and tariff uncertainty create sector-wide pressure.",
    ...
    p.gainP<0?"Current loss of "+p.gainP.toFixed(1)+"% suggests thesis under pressure ÃÂ¢ÃÂÃÂ review catalyst timeline.":"Taking profits on "+p.gainP.toFixed(1)+"% gain may be tactically sound given macro risks.",
  ];
  return (
    <div className="fi" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px"}}>
      <div style={{background:"#001408",border:"1px solid "+CG+"33",padding:"10px"}}>
        <div style={{fontFamily:"Orbitron",fontSize:"9px",color:CG,letterSpacing:"2px",marginBottom:"8px"}}>ÃÂ°ÃÂÃÂÃÂ BULL CASE</div>
        {bullPoints.map(function(pt,i){return <div key={i} style={{fontSize:"11px",color:CB,lineHeight:1.65,marginBottom:"5px"}}>ÃÂ¢ÃÂÃÂ¸ {pt}</div>;})}
      </div>
      <div style={{background:"#140000",border:"1px solid "+CR+"33",padding:"10px"}}>
        <div style={{fontFamily:"Orbitron",fontSize:"9px",color:CR,letterSpacing:"2px",marginBottom:"8px"}}>ÃÂ°ÃÂÃÂÃÂ» BEAR CASE</div>
        {bearPoints.map(function(pt,i){return <div key={i} style={{fontSize:"11px",color:CB,lineHeight:1.65,marginBottom:"5px"}}>ÃÂ¢ÃÂÃÂ¸ {pt}</div>;})}
      </div>
      <div style={{gridColumn:"1/-1",padding:"8px",background:sig.color+"11",border:"1px solid "+sig.color+"33",textAlign:"center"}}>
        <span style={{fontFamily:"Orbitron",fontSize:"10px",color:sig.color,letterSpacing:"2px"}}>MARCUS VERDICT: {sig.sig} ÃÂ¢ÃÂÃÂª CONVICTION {sig.score}/10</span>
      </div>
    </div>
  );
}

// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ SOUN OPS TAB ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      ...
      <div style={{background:BP,border:"1px solid "+CA+"33",padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div>
          ...
          <div style={{fontSize:"10px",color:CC,marginTop:"2px",letterSpacing:"1px"}}>SOUNDHOUND AI ÃÂ¢ÃÂÃÂª {SOUN_DATA.sector}</div>
        </div>
        <div style={{textAlign:"right"}}>
          <div style={{fontFamily:"Orbitron",fontSize:"22px",fontWeight:900,color:CB}}>${sounPrice.toFixed(2)}</div>
          <div style={{fontSize:"10px",color:CD,marginTop:"2px"}}>BREAKEVEN GAP: +{((l1.breakeven-sounPrice)/sounPrice*100).toFixed(1)}% ÃÂ¢ÃÂÃÂ +{((l2.breakeven-sounPrice)/sounPrice*100).toFixed(1)}%</div>
        </div>
      </div>
      ...
      {activeSection==="theta"&&(
        <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
          <Panel label={"ÃÂ¢ÃÂÃÂ THETA DECAY CLOCK ÃÂ¢ÃÂÃÂ DAILY PREMIUM BLEED"}>
            <div style={{fontSize:"10px",color:CD,marginBottom:"12px",letterSpacing:"1px"}}>Every day that passes, your options lose value from time decay. This is theta ÃÂ¢ÃÂÃÂ your cost of holding.</div>
            {[{l:l1,theta:theta1,days:days1,label:"1/15/27 LEAPS"},{l:l2,theta:theta2,days:days2,label:"1/21/28 LEAPS"}].map(function(item,i){return (
              <div key={i} style={{background:BD,border:"1px solid #1a1520",padding:"12px",marginBottom:"8px"}}>
                ...
                <div style={{fontSize:"10px",color:CD,lineHeight:1.6}}>
                  At current decay rate, you lose approximately <span style={{color:CY}}>${(item.theta.dailyDecay*30).toFixed(0)}/month</span> on this position while waiting for SOUN to move. The 2028 LEAPS bleeds slower per day ÃÂ¢ÃÂÃÂ better for long-duration holds.
                </div>
              </div>
            );})}
          </Panel>
      ...
      {activeSection==="sentiment"&&(
        <Panel label={"ÃÂ¢ÃÂÃÂ SOUN SENTIMENT ANALYSIS"}>
          <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
            ...
            <div style={{padding:"10px",background:"#001808",border:"1px solid "+CG+"22"}}>
              <div style={{fontFamily:"Orbitron",fontSize:"10px",color:CG,letterSpacing:"2px",marginBottom:"6px"}}>ÃÂ°ÃÂÃÂÃÂ BULL THESIS</div>
            ...
            <div style={{padding:"10px",background:"#180000",border:"1px solid "+CR+"22"}}>
              <div style={{fontFamily:"Orbitron",fontSize:"10px",color:CR,letterSpacing:"2px",marginBottom:"6px"}}>ÃÂ°ÃÂÃÂÃÂ» BEAR THESIS</div>
      ...
      {activeSection==="catalysts"&&(
        <Panel label={"ÃÂ¢ÃÂÃÂ SOUN CATALYST CALENDAR"}>
      ...
      {/* Risks Section */}
      {activeSection==="risks"&&(
        <Panel label={"ÃÂ¢ÃÂÃÂ SOUN RISK ASSESSMENT"}>
          <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
            {SOUN_DATA.risks.map(function(r,i){return (
              <div key={i} style={{display:"flex",gap:"10px",padding:"10px",background:"#140000",border:"1px solid "+CR+"22"}}>
                <span style={{color:CR,fontSize:"12px",flexShrink:0}}>ÃÂ¢ÃÂÃÂ </span>
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ VOICE MODE ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
function VoiceMode({alf,enriched,fg,onClose}) {
  ...
  function startListening() {
    ...
    rec.onerror = function() { setStatus("idle"); setResponse("I apologize, sir ÃÂ¢ÃÂÃÂ the microphone appears to be unavailable."); setStatus("response"); };
    rec.start();
    setStatus("listening");
    setTranscript("");
    setResponse("");
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(0,0,0,.9)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"20px",padding:"24px"}}>
      <div style={{fontFamily:"Orbitron",fontSize:"14px",color:CA,letterSpacing:"5px"}}>MARCUS ÃÂ¢ÃÂÃÂ VOICE INTERFACE</div>
      ...
      <button onClick={status==="idle"||status==="response"?startListening:stopSpeaking}
        ...
        {status==="listening"?"ÃÂ°ÃÂÃÂÃÂ´":status==="speaking"?"ÃÂ°ÃÂÃÂÃÂ":"ÃÂ°ÃÂÃÂÃÂ"}
      </button>
      ...
      {response&&<div style={{background:BP,border:"1px solid "+CA+"33",padding:"12px 16px",maxWidth:"320px",width:"100%"}}>
        <div style={{fontSize:"9px",color:CD,letterSpacing:"2px",marginBottom:"6px"}}>ÃÂ¢ÃÂÃÂ MARCUS ÃÂ¢ÃÂÃÂª APEX</div>
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ PARLAYS TAB ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ ENHANCED SCOUT ENGINE ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ

// 2026 Vegas preseason win totals (O/U lines ÃÂ¢ÃÂÃÂ best early-season predictor)
var VEGAS_WINS = {
...
// Composite playoff probability ÃÂ¢ÃÂÃÂ weighted for early season
function computePlayoffScore(team, streaks) {
  ...
  // 2. PYTHAGOREAN EXPECTED WINS (25% weight ÃÂ¢ÃÂÃÂ better than actual W-L)
...
function getLegSt(team,type,mlbData){
  ...
  if(tracking&&s.conf>=75)return{color:CG,icon:"ÃÂ¢ÃÂÃÂ",conf:s.conf,label:"ON TRACK",w:s.w,l:s.l};
  if(tracking&&s.conf>=55)return{color:CY,icon:"~",conf:s.conf,label:"LIKELY",w:s.w,l:s.l};
  if(!tracking&&s.conf>=75)return{color:CR,icon:"ÃÂ¢ÃÂÃÂ",conf:s.conf,label:"AT RISK",w:s.w,l:s.l};
...
function TabParlays(){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      ...
      {!mlbLoading&&<div style={{background:"#08080f",border:"1px solid "+CG+"33",padding:"6px 12px",fontSize:"9px",color:CG,fontFamily:"Orbitron",letterSpacing:"1px"}}>ÃÂ¢ÃÂÃÂ LIVE STANDINGS ÃÂ¢ÃÂÃÂ UPDATED NOW ÃÂ¢ÃÂÃÂª MLB STATS API</div>}

      {/* Universal legs */}
      <div style={{background:"#08080f",border:"1px solid "+CA+"44",padding:"10px 12px"}}>
        <div style={{fontSize:"9px",color:CA,fontFamily:"Orbitron",letterSpacing:"2px",marginBottom:"8px"}}>ÃÂ¢ÃÂÃÂ¡ UNIVERSAL LEGS ÃÂ¢ÃÂÃÂ IN ALL 4 PARLAYS</div>
        ...
        {PARLAY_DATA.map(function(pd,i){
          ...
          return(
            <button key={i} onClick={function(){setAp(i);}} style={{flex:1,background:"transparent",border:"1px solid "+(ap===i?CA:"#2a1e08"),padding:"8px 4px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:"2px",transition:"all .2s"}}>
              ...
              <span style={{fontSize:"9px",color:CD}}>${pd.stake}ÃÂ¢ÃÂÃÂ${pd.payout.toLocaleString()}</span>
      ...
      {/* OVERVIEW */}
      {view==="overview"&&(
        <Panel label={"ÃÂ¢ÃÂÃÂ "+p.odds+" ÃÂ¢ÃÂÃÂ OVERVIEW & PROBABILITY"}>
          ...
          <div style={{fontSize:"9px",color:CD,lineHeight:1.7}}>ÃÂ¢ÃÂÃÂ  Parlays 2 &amp; 4 have conflicting CIN legs ÃÂ¢ÃÂÃÂ an intentional hedge ensuring at least one parlay survives either outcome.</div>
        </Panel>
      )}

      {/* LEG STATUS */}
      {view==="legs"&&(
        <Panel label={"ÃÂ¢ÃÂÃÂ "+p.odds+" ÃÂ¢ÃÂÃÂ ALL "+p.legs+" LEGS"}>
          <div style={{marginBottom:"12px"}}>
            <div style={{fontFamily:"Orbitron",fontSize:"9px",color:CG,letterSpacing:"2px",marginBottom:"8px"}}>ÃÂ¢ÃÂÃÂ MAKE PLAYOFFS ({p.make.length})</div>
              ...
              {p.make.map(function(t){
                ...
                return(
                  <div key={t} style={{padding:"6px 10px",background:st.color+"12",border:"1px solid "+st.color+"44",minWidth:"76px"}}>
                    ...
                    <div style={{fontSize:"9px",color:CD,marginTop:"2px"}}>{st.w}-{st.l} ÃÂ¢ÃÂÃÂª {st.conf}%</div>
          ...
          <div>
            <div style={{fontFamily:"Orbitron",fontSize:"9px",color:CR,letterSpacing:"2px",marginBottom:"8px"}}>ÃÂ¢ÃÂÃÂ MISS PLAYOFFS ({p.miss.length})</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:"5px"}}>
      ...
      {/* STANDINGS */}
      {view==="standings"&&(
        <Panel label={"ÃÂ¢ÃÂÃÂ 2026 MLB STANDINGS ÃÂ¢ÃÂÃÂ PLAYOFF PROJECTION"}>
            ...
            return (
              <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                <div style={{padding:"8px 12px",background:"#08080f",border:"1px solid #1a1520",fontSize:"9px",color:CD,lineHeight:1.7}}>
                  ...
                  <span style={{color:CG}}> ÃÂ°ÃÂÃÂÃÂ¥ = 7+ wins last 10. </span>
                  <span style={{color:CR}}> ÃÂ¢ÃÂÃÂ = 3 or fewer last 10. </span>
                  <span style={{color:CA}}> GOLD border = your parlay leg.</span>
                </div>

                <Panel label={"ÃÂ¢ÃÂÃÂ MOST LIKELY TO MAKE PLAYOFFS ÃÂ¢ÃÂÃÂ TOP "+makeTeams.length+" TEAMS"}>
                ...
                <Panel label={"ÃÂ¢ÃÂÃÂ MOST LIKELY TO MISS PLAYOFFS ÃÂ¢ÃÂÃÂ BOTTOM "+missTeams.length+" TEAMS"}>
                ...
                <Panel label={"ÃÂ¢ÃÂÃÂ MARCUS ÃÂ¢ÃÂÃÂ RECOMMENDED NEXT PARLAY LEGS"}>
                      ...
                      {makeTeams.slice(0,4).map(function(t){return(
                        <div key={t.t+"m"} style={{display:"flex",justifyContent:"space-between",padding:"6px 8px",marginBottom:"3px",background:"rgba(24,201,58,.06)",border:"1px solid "+CG+"22"}}>
                          <div>
                            ...
                            <span style={{fontSize:"9px",color:CD,marginLeft:"6px"}}>{t.w}-{t.l} ÃÂ¢ÃÂÃÂª {t.vegasWins}W O/U</span>
                          </div>
                  ...
                  <div style={{padding:"10px",background:"#08080f",border:"1px solid "+CA+"33"}}>
                    <div style={{fontFamily:"Orbitron",fontSize:"9px",color:CA,letterSpacing:"2px",marginBottom:"6px"}}>MARCUS ÃÂ¢ÃÂÃÂ CONSTRUCTION ADVICE</div>
                    <div style={{fontSize:"10px",color:CB,lineHeight:1.8}}>
                      Stack your highest-score MAKE legs with your lowest-score MISS legs. Legs above 80% are your anchors ÃÂ¢ÃÂÃÂ build around them.
                      Legs 60-79% are strong value adds. Avoid legs 50-60% ÃÂ¢ÃÂÃÂ too close to call this early.
      ...
      {/* CALCULATOR */}
      {view==="calc"&&(
        <Panel label={"ÃÂ¢ÃÂÃÂ CASH OUT CALCULATOR ÃÂ¢ÃÂÃÂ ALL 4 PARLAYS"}>
            ...
            return(
              <div key={pd.id} style={{background:BD,border:"1px solid #1a1520",padding:"12px",marginBottom:"8px"}}>
                ...
                <div style={{fontSize:"9px",color:CD,marginTop:"5px"}}>
                  Verdict: <span style={{color:pr>2?CY:CR}}>{pr>2?"HOLD ÃÂ¢ÃÂÃÂ season too early to cash out":"MONITOR ÃÂ¢ÃÂÃÂ assess leg risk mid-season"}</span>
                </div>
              </div>
            );
          })}
          <div style={{padding:"10px",background:"#08080f",border:"1px solid "+CA+"33"}}>
            <div style={{fontFamily:"Orbitron",fontSize:"9px",color:CA,letterSpacing:"2px",marginBottom:"6px"}}>MARCUS ÃÂ¢ÃÂÃÂ PARLAY ASSESSMENT</div>
            <div style={{fontSize:"10px",color:CB,lineHeight:1.8}}>$112 at risk against a $13,009 maximum return. The CIN hedge across parlays 2 and 4 is rational construction. Your five universal legs ÃÂ¢ÃÂÃÂ Yankees, Mariners, Braves making playoffs; Angels and Astros missing ÃÂ¢ÃÂÃÂ are the critical path. Monitor those above all others. It is early. Patience is the correct posture.</div>
          </div>
        </Panel>
      )}
    </div>
...
function StockPopup({ticker,price,cost,shares,gainP,chgP,onClose}){
  ...
  var mktVal=parseFloat(price)*shares;
  var costVal=parseFloat(cost)*shares;
  var totalGain=mktVal-costVal;
  var rec=gainP>20?"HOLD ÃÂ¢ÃÂÃÂ STRONG POSITION":gainP<-20?"REVIEW ÃÂ¢ÃÂÃÂ THESIS CHECK":chgP<-5?"WATCH ÃÂ¢ÃÂÃÂ SHARP DECLINE":chgP>5?"TRIM ÃÂ¢ÃÂÃÂ CONSIDER LOCKING GAINS":"HOLD ÃÂ¢ÃÂÃÂ WITHIN NORMAL RANGE";
  var color=gainP>0?CG:CR;
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(3,3,8,.93)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:BP,border:"2px solid "+CA+"55",padding:"20px",maxWidth:"460px",width:"92%",maxHeight:"82vh",overflowY:"auto"}} onClick={function(e){e.stopPropagation();}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"14px"}}>
          <div>
            <div style={{fontFamily:"Orbitron",fontSize:"24px",fontWeight:900,color:CA}}>{ticker}</div>
            <div style={{fontSize:"10px",color:CD,letterSpacing:"2px",marginTop:"2px"}}>POSITION ANALYSIS</div>
          </div>
          <button className="xbtn" onClick={onClose}>ÃÂÃÂ</button>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"6px",marginBottom:"12px"}}>
          {[["PRICE","$"+parseFloat(price).toFixed(2),CB],["TODAY",(tUp?"+":"")+parseFloat(chgP).toFixed(2)+"%",tUp?CG:CR],["TOTAL",(up?"+":"")+parseFloat(gainP).toFixed(2)+"%",color]].map(function(row,ri){
            return(<div key={ri} style={{background:BD,padding:"10px",textAlign:"center",border:"1px solid #1a1520"}}><div style={{fontSize:"8px",color:CD,letterSpacing:"1px",marginBottom:"3px"}}>{row[0]}</div><div style={{fontFamily:"Orbitron",fontSize:"13px",fontWeight:700,color:row[2]}}>{row[1]}</div></div>);
          })}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px",marginBottom:"12px"}}>
          {[["SHARES",parseFloat(shares).toFixed(3)],["AVG COST","$"+parseFloat(cost).toFixed(2)],["MKT VALUE","$"+mktVal.toFixed(2)],["TOTAL P&L",(totalGain>=0?"+$":"-$")+Math.abs(totalGain).toFixed(2)]].map(function(row,ri){
            return(<div key={ri} style={{background:BD,padding:"8px",border:"1px solid #1a1520"}}><div style={{fontSize:"9px",color:CD}}>{row[0]}</div><div style={{fontFamily:"Orbitron",fontSize:"11px",color:CB,marginTop:"2px"}}>{row[1]}</div></div>);
          })}
        </div>
        <div style={{padding:"12px",background:"rgba(240,163,10,.06)",border:"1px solid "+CA+"33",marginBottom:"10px"}}>
          <div style={{fontFamily:"Orbitron",fontSize:"9px",color:CA,letterSpacing:"2px",marginBottom:"6px"}}>MARCUS ÃÂ¢ÃÂÃÂ {rec}</div>
          <div style={{fontSize:"10px",color:CC,lineHeight:1.8}}>{gainP>20?"Strong total return. Let this winner run unless it exceeds 25% of portfolio.":gainP<-20?"Position is significantly underwater. Re-evaluate if original thesis still holds.":chgP<-5?"Sharp single-day drop. Check for news before acting ÃÂ¢ÃÂÃÂ likely noise.":chgP>5?"Strong day. Consider trimming a small portion to lock in gains if overweight.":"Position within normal range. No immediate action required."}</div>
        </div>
        <div style={{fontSize:"9px",color:CD,lineHeight:1.6}}>Position sizing: at {parseFloat(shares).toFixed(3)} shares your exposure is ${mktVal.toFixed(2)}. Standard sizing suggests no single name exceed 20% of portfolio value.</div>
      </div>
    </div>
  );
}
...
function ErrMsg({text}) {
  return <div style={{color:CR,fontSize:"10px",letterSpacing:"1px"}}>ÃÂ¢ÃÂÃÂ  {text}</div>;
}
