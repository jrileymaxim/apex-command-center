...
function TabBriefing({alf,onAlf,wx,city,setCity,onWx,fg,enriched,totP,eod,onEod}) {
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
      ...
      <Panel label={"◈ MARCUS — END OF DAY BRIEFING"} right={<button className="btn bsm" onClick={onEod}>GENERATE</button>}>
        {!eod&&<div style={{fontSize:"10px",color:CD,lineHeight:1.8}}>Reads live portfolio prices — real data, no web search. Refresh Portfolio first then hit GENERATE.</div>}
        {eod&&(
          <div className="fi">
            ...
            <div style={{marginTop:"10px",fontSize:"9px",color:CD,letterSpacing:"2px"}}>— MARCUS ▪ LIVE DATA</div>
          </div>
        )}
      </Panel>
      ...
      <Panel label={"◈ POSITIONS — "+POSITIONS.length+" HOLDINGS "+(live?"▪ LIVE":"▪ SESSION DATA 4/14")} right={<button className="btn bsm" onClick={onRefresh} disabled={status==="loading"}>↻ REFRESH</button>}>
        ...
        {status!=="loading"&&enriched.map(function(p,i){return (
          <div key={p.ticker} className="pr si" style={{animationDelay:i*30+"ms"}}>
            ...
            <Panel label={"◈ PORTFOLIO TARGET CALCULATOR"}>
...
function TabAlerts({filings,loading,enabled,onEnable,onScan}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      <Panel label={"◈ EDGAR + ALERTS"}>
        <div style={{display:"flex",gap:"8px",marginBottom:"10px"}}>
          <button className="btn" style={{flex:1}} onClick={onScan}>{loading?"SCANNING...":"↻ SCAN SEC EDGAR"}</button>
          <button className="btn" style={{flex:1,color:enabled?CG:CA,borderColor:enabled?CG:CA}} onClick={onEnable}>{enabled?"🔔 ON":"🔕 ENABLE"}</button>
        </div>
      ...
      {urgent.length>0&&(
        <Panel label={"◈ URGENT — "+urgent.length+" FILING"+(urgent.length!==1?"S":"")}>
            ...
            <div key={fi} style={{padding:"10px",marginBottom:"6px",background:"rgba(224,48,16,.06)",border:"1px solid "+CR+"44"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"4px"}}>
                <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                  ...
                  <span style={{fontFamily:"Orbitron",fontSize:"8px",color:CR}}>⚠ URGENT</span>
                </div>
      ...
      <Panel label={"◈ SEC EDGAR — LAST 7 DAYS"+(filings.length>0?" ("+filings.length+")":"")}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
        ...
        <Panel label={"◈ PARLAYS SUMMARY"}>
          ...
          <div style={{marginTop:"8px",fontSize:"9px",color:CD,letterSpacing:"1px"}}>AT RISK: $112 ▪ MAX PAYOUT: $13,008.93 ▪ SETTLES OCT 31</div>
            ...
            <Panel label={"◈ MARCUS PREDICTION LOG"}>
        <div style={{marginBottom:"10px"}}>
          <div style={{fontSize:"9px",color:CD,letterSpacing:"1px",marginBottom:"6px"}}>LOG A CALL — TRACK YOUR ACCURACY</div>
        ...
        {predLog.length>0&&(
          <div>
            <div style={{fontSize:"9px",color:CD,letterSpacing:"1px",marginBottom:"6px"}}>ACCURACY: {(predLog.filter(function(p){return p.outcome==="WIN";}).length/Math.max(1,predLog.filter(function(p){return p.outcome!=="OPEN";}).length)*100).toFixed(0)}% — {predLog.filter(function(p){return p.outcome==="WIN";}).length}W / {predLog.filter(function(p){return p.outcome==="LOSS";}).length}L / {predLog.filter(function(p){return p.outcome==="OPEN";}).length} OPEN</div>
                ...
                <div key={pred.id} style={{padding:"8px",marginBottom:"4px",background:BD,border:"1px solid #1a1520"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"3px"}}>
                    ...
                    <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                      ...
                      <button style={{background:"transparent",border:"none",color:CD,cursor:"pointer",fontSize:"12px"}} onClick={function(){var updated=predLog.map(function(p){return p.id===pred.id?Object.assign({},p,{outcome:p.outcome==="WIN"?"LOSS":p.outcome==="LOSS"?"OPEN":"WIN"}):p;});setPredLog(updated);localStorage.setItem("apex-predictions",JSON.stringify(updated));}}>↻</button>
                      <button style={{background:"transparent",border:"none",color:CD,cursor:"pointer",fontSize:"12px"}} onClick={function(){var updated=predLog.filter(function(p){return p.id!==pred.id;});setPredLog(updated);localStorage.setItem("apex-predictions",JSON.stringify(updated));}}>×</button>
                    </div>
                  </div>
                  <div style={{fontSize:"9px",color:CD}}>Entry: ${pred.entryPrice.toFixed(2)}{pred.targetPrice>0?" → $"+pred.targetPrice.toFixed(2):""} {pred.notes?"▸ "+pred.notes.slice(0,40):""} <span style={{color:CC}}>{pred.date}</span></div>
                </div>
              );
            })}
          </div>
...
function TabIntel({enriched,status,totP,fg}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      <Panel label={"◈ POSITION SIGNALS — LIVE DRIVEN"}>
...
function StockPopup({ticker,price,cost,shares,gainP,chgP,onClose}){
  ...
  var rec=gainP>20?"HOLD — STRONG":gainP<-20?"REVIEW — THESIS CHECK":chgP<-5?"WATCH — SHARP DROP":chgP>5?"CONSIDER TRIM":"HOLD — NORMAL";
