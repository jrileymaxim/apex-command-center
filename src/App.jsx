...
async function fetchPrices(){
  ...
  try{
    ...
    console.warn("fetchPrices fallback:",e.message);
    return FALLBACK;
  }
}
...
function generateEOD(enriched,totCost,totMkt,totGain,totP,bench,fg){
  ...
  var now=new Date(),ds=now.toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"}),ts=now.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:true});
  ...
  var upCt=wd.filter(function(p){return p.chgP>0;}).length,dnCt=wd.filter(function(p){return p.chgP<0;}).length;
  ...
  var lines=["APEX EOD â "+ds+" at "+ts,"","Portfolio: $"+totMkt.toFixed(2)+" / $"+totCost.toFixed(2)+" cost. Return: "+(totGain>=0?"+":"")+totP.toFixed(2)+"% ($"+(totGain>=0?"+":"")+totGain.toFixed(2)+").",upCt+" up, "+dnCt+" down. Est. day P&L: "+(dayDollar>=0?"+":"")+dayDollar.toFixed(2)+".","",(top?"Best: "+top.ticker+" "+(top.chgP>0?"+":"")+top.chgP.toFixed(2)+"% at $"+top.price+".":""),  (bot&&bot.chgP<0?"Worst: "+bot.ticker+" "+bot.chgP.toFixed(2)+"% at $"+bot.price+".":""),"","AI cluster: "+aiUp+" up, "+aiDn+" down.",(bd.SPY?"Market: SPY "+bd.SPY.chg+"%":""  ),"","Sentiment: "+(fg&&fg.label||"Neutral")+". Stay disciplined."];
  ...
  return (
    <div style={{height:"100vh",overflow:"hidden",background:BG,color:CC,fontFamily:"'Share Tech Mono',monospace",display:"flex",flexDirection:"column",position:"relative"}} className={alertOn?"alertMode":""}>
      ...
      <div style={{flex:1,minHeight:0,overflow:"auto",position:"relative",zIndex:1,padding:"12px 14px"}}>
        ...
        {tab==="alerts"    && <TabAlerts filings={edgarFilings} loading={edgarLoading} enabled={alertsEnabled} onEnable={async function(){if("Notification" in window){var p=await Notification.requestPermission();setAlertsEnabled(p==="granted");}}} onScan={async function(){setEdgarLoading(true);var f=await fetchEdgarFilings();setEdgarFilings(f);setEdgarLoading(false);}}/>}
...
function TabBriefing({alf,onAlf,wx,city,setCity,onWx,fg,enriched,totP,eod,onEod}) {
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
      ...
      <Panel label={"â MARCUS â END OF DAY BRIEFING"} right={<button className="btn bsm" onClick={onEod}>GENERATE</button>}>
        {!eod&&<div style={{fontSize:"10px",color:CD,lineHeight:1.8}}>Reads live portfolio prices â real data, no web search. Refresh Portfolio first then hit GENERATE.</div>}
        {eod&&(
          <div className="fi">
            ...
            <div style={{marginTop:"10px",fontSize:"9px",color:CD,letterSpacing:"2px"}}>â MARCUS âª LIVE DATA</div>
          </div>
        )}
      </Panel>
      ...
      <Panel label={"â POSITIONS â "+POSITIONS.length+" HOLDINGS "+(live?"âª LIVE":"âª SESSION DATA 4/14")} right={<button className="btn bsm" onClick={onRefresh} disabled={status==="loading"}>â» REFRESH</button>}>
        ...
        {status!=="loading"&&enriched.map(function(p,i){return (
          <div key={p.ticker} className="pr si" style={{animationDelay:i*30+"ms"}}>
            ...
            <Panel label={"â PORTFOLIO TARGET CALCULATOR"}>
        <div style={{display:"flex",gap:"8px",marginBottom:"10px"}}>
          <div style={{flex:1}}><div style={{fontSize:"9px",color:CD,marginBottom:"4px",letterSpacing:"1px"}}>GOAL ($)</div><input className="inp" value={tgtAmt} onChange={function(e){setTgtAmt(e.target.value);setTgtResult(null);}} placeholder="50000"/></div>
          <div style={{flex:1}}><div style={{fontSize:"9px",color:CD,marginBottom:"4px",letterSpacing:"1px"}}>DAYS</div><input className="inp" value={tgtDays} onChange={function(e){setTgtDays(e.target.value);setTgtResult(null);}} placeholder="365"/></div>
        </div>
        <button className="btn" style={{width:"100%",marginBottom:"10px"}} onClick={function(){
          ...
          var buys=ranked.slice(0,4).map(function(px){var dollar=need*0.25,shs=dollar/(parseFloat(px.price)||px.avgCost);return {t:px.ticker,p:(parseFloat(px.price)||px.avgCost).toFixed(2),d:dollar.toFixed(2),s:shs.toFixed(3)};});
        ...
        {tgtResult&&(
          <div className="fi">
            ...
            {tgtResult.buys.map(function(bx,bi){return(<div key={bi} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #0c0a14"}}><span style={{fontFamily:"Orbitron",fontSize:"11px",color:CA}}>{bx.t}</span><span style={{fontSize:"10px",color:CB}}>{bx.s} sh @ ${bx.p}</span><span style={{fontFamily:"Orbitron",fontSize:"10px",color:CG}}>${bx.d}</span></div>);})}
          </div>
        )}
      </Panel>
    ...
    try{
      ...
      for(var i=0;i<Math.min(forms.length,10);i++){
        if(dates[i]>=cutoff&&["8-K","4","SC 13D","SC 13G","10-Q","10-K"].indexOf(forms[i])>-1){
          all.push({ticker:ticker,form:forms[i],date:dates[i],urgent:forms[i]==="8-K"||forms[i]==="4",url:"https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK="+cik+"&type="+forms[i]+"&dateb=&owner=include&count=5"});
        }
      }
    }catch(e){}
  }));
...
function TabAlerts({filings,loading,enabled,onEnable,onScan}){
  ...
  var fl=function(form){var m={"8-K":"MATERIAL EVENT","4":"INSIDER TRADE","10-Q":"QUARTERLY","10-K":"ANNUAL","SC 13D":"ACTIVIST","SC 13G":"LARGE HOLDER"};return m[form]||form;};
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      <Panel label={"â EDGAR + ALERTS"}>
        <div style={{display:"flex",gap:"8px",marginBottom:"10px"}}>
          <button className="btn" style={{flex:1}} onClick={onScan}>{loading?"SCANNING...":"â» SCAN SEC EDGAR"}</button>
          <button className="btn" style={{flex:1,color:enabled?CG:CA,borderColor:enabled?CG:CA}} onClick={onEnable}>{enabled?"ð ON":"ð ENABLE"}</button>
        </div>
        <div style={{fontSize:"9px",color:CD,lineHeight:1.7}}>Scans SEC EDGAR for 8-K material events, Form 4 insider trades, SC 13D activist filings for all positions. GitHub Actions monitor also runs every 5 min and sends ntfy alerts to your phone.</div>
      </Panel>
      {urgent.length>0&&(
        <Panel label={"â URGENT â "+urgent.length+" FILING"+(urgent.length!==1?"S":"")}>
          {urgent.map(function(f,fi){return(
            <div key={fi} style={{padding:"10px",marginBottom:"6px",background:"rgba(224,48,16,.06)",border:"1px solid "+CR+"44"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"4px"}}>
                <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                  ...
                  <span style={{fontFamily:"Orbitron",fontSize:"8px",padding:"2px 6px",background:fc(f.form)+"22",color:fc(f.form),border:"1px solid "+fc(f.form)+"44"}}>{fl(f.form)}</span>
                  <span style={{fontFamily:"Orbitron",fontSize:"8px",color:CR}}>â  URGENT</span>
                </div>
                <span style={{fontSize:"9px",color:CD}}>{f.date}</span>
              </div>
              <a href={f.url} target="_blank" rel="noreferrer" style={{fontSize:"9px",color:CA,textDecoration:"none"}}>â¸ VIEW ON SEC.GOV â</a>
            </div>
          );})}        
        </Panel>
      )}
      <Panel label={"â SEC EDGAR â LAST 7 DAYS"+(filings.length>0?" ("+filings.length+")":"")}>
        ...
        {!loading&&normal.map(function(f,fi){return(
          <div key={fi} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:"1px solid #0c0a14"}}>
            <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
              ...
              <span style={{fontFamily:"Orbitron",fontSize:"8px",padding:"1px 5px",background:fc(f.form)+"22",color:fc(f.form),border:"1px solid "+fc(f.form)+"33"}}>{fl(f.form)}</span>
            </div>
            <div style={{display:"flex",gap:"10px",alignItems:"center"}}>
              <span style={{fontSize:"9px",color:CD}}>{f.date}</span>
              <a href={f.url} target="_blank" rel="noreferrer" style={{fontSize:"9px",color:CC,textDecoration:"none"}}>VIEW â</a>
            </div>
          </div>
        );})}        
      </Panel>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
        ...
        <Panel label={"â PARLAYS SUMMARY"}>
          ...
          <div style={{marginTop:"8px",fontSize:"9px",color:CD,letterSpacing:"1px"}}>AT RISK: $112 âª MAX PAYOUT: $13,008.93 âª SETTLES OCT 31</div>
            ...
            <Panel label={"â MARCUS PREDICTION LOG"}>
        <div style={{marginBottom:"10px"}}>
          <div style={{fontSize:"9px",color:CD,letterSpacing:"1px",marginBottom:"6px"}}>LOG A CALL â TRACK YOUR ACCURACY</div>
          <div style={{display:"flex",gap:"6px",marginBottom:"6px"}}>
            <select style={{flex:1,background:BD,color:CB,border:"1px solid #2a1e08",padding:"6px",fontFamily:"Share Tech Mono,monospace",fontSize:"11px"}} value={predTicker} onChange={function(e){setPredTicker(e.target.value);}}>
              <option value="">TICKER...</option>
              {POSITIONS.map(function(pos){return <option key={pos.ticker} value={pos.ticker}>{pos.ticker}</option>;})}
            </select>
            <select style={{width:"90px",background:BD,color:CB,border:"1px solid #2a1e08",padding:"6px",fontFamily:"Share Tech Mono,monospace",fontSize:"11px"}} value={predCall} onChange={function(e){setPredCall(e.target.value);}}>
              <option>BUY</option>
              <option>SELL</option>
              <option>HOLD</option>
            </select>
          ...
          <div style={{display:"flex",gap:"6px",marginBottom:"6px"}}>
            <input className="inp" style={{flex:1}} value={predTarget} onChange={function(e){setPredTarget(e.target.value);}} placeholder="TARGET ($)"/>
          ...
          <button className="btn" style={{width:"100%"}} onClick={function(){
            ...
            var pos=safeEnriched.find(function(p){return p.ticker===predTicker;});
        ...
        {predLog.length===0&&<div style={{fontSize:"10px",color:CD}}>No predictions yet. Log your first call above.</div>}
        {predLog.length>0&&(
          <div>
            <div style={{fontSize:"9px",color:CD,letterSpacing:"1px",marginBottom:"6px"}}>ACCURACY: {(predLog.filter(function(p){return p.outcome==="WIN";}).length/Math.max(1,predLog.filter(function(p){return p.outcome!=="OPEN";}).length)*100).toFixed(0)}% â {predLog.filter(function(p){return p.outcome==="WIN";}).length}W / {predLog.filter(function(p){return p.outcome==="LOSS";}).length}L / {predLog.filter(function(p){return p.outcome==="OPEN";}).length} OPEN</div>
            {predLog.slice(0,8).map(function(pred,pi){
              var oc=pred.outcome==="WIN"?CG:pred.outcome==="LOSS"?CR:CY;
              var pos2=safeEnriched.find(function(p){return p.ticker===pred.ticker;});
              var cur2=pos2?parseFloat(pos2.price||pos2.avgCost):null;
              var pnl=cur2&&pred.entryPrice?((cur2-pred.entryPrice)/pred.entryPrice*100).toFixed(1):null;
              return(
                <div key={pred.id} style={{padding:"8px",marginBottom:"4px",background:BD,border:"1px solid #1a1520"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"3px"}}>
                    <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                      ...
                      <span style={{fontFamily:"Orbitron",fontSize:"8px",padding:"1px 5px",background:oc+"22",color:oc,border:"1px solid "+oc+"44"}}>{pred.outcome}</span>
                    </div>
                    <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                      {pnl&&<span style={{fontFamily:"Orbitron",fontSize:"10px",color:parseFloat(pnl)>0?CG:CR}}>{parseFloat(pnl)>0?"+":""}{pnl}%</span>}
                      <button style={{background:"transparent",border:"none",color:CD,cursor:"pointer",fontSize:"12px"}} onClick={function(){var updated=predLog.map(function(p){return p.id===pred.id?Object.assign({},p,{outcome:p.outcome==="WIN"?"LOSS":p.outcome==="LOSS"?"OPEN":"WIN"}):p;});setPredLog(updated);localStorage.setItem("apex-predictions",JSON.stringify(updated));}}>â»</button>
                      <button style={{background:"transparent",border:"none",color:CD,cursor:"pointer",fontSize:"12px"}} onClick={function(){var updated=predLog.filter(function(p){return p.id!==pred.id;});setPredLog(updated);localStorage.setItem("apex-predictions",JSON.stringify(updated));}}>Ã</button>
                    </div>
                  </div>
                  <div style={{fontSize:"9px",color:CD}}>Entry: ${pred.entryPrice.toFixed(2)}{pred.targetPrice>0?" â $"+pred.targetPrice.toFixed(2):""} {pred.notes?"â¸ "+pred.notes.slice(0,40):""} <span style={{color:CC}}>{pred.date}</span></div>
                </div>
              );
            })}
          </div>
...
function TabIntel({enriched,status,totP,fg}){
  ...
  var signals=enriched.map(function(p){var chg=parseFloat(p.chgP)||0,gain=p.gainP||0;var momentum=chg>2?"BULLISH":chg<-2?"BEARISH":"NEUTRAL";var posture=gain>15?"STRONG":gain>0?"POSITIVE":gain>-10?"CAUTION":"WEAK";var signal=(momentum==="BULLISH"&&posture!=="WEAK")?"BUY":(momentum==="BEARISH"||posture==="WEAK")?"SELL":"HOLD";var conf=Math.min(95,Math.max(25,Math.abs(chg)*15+Math.abs(gain)*0.5+40));return {ticker:p.ticker,signal:signal,conf:conf.toFixed(0),chg:chg.toFixed(2),gain:gain.toFixed(1),price:(parseFloat(p.price)||p.avgCost).toFixed(2)};});
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      <Panel label={"â POSITION SIGNALS â LIVE DRIVEN"}>
        {status==="loading"&&<Spinner label="LOADING..."/>}
        {status!=="loading"&&signals.map(function(s,si){return(
          <div key={si} style={{display:"flex",alignItems:"center",gap:"8px",padding:"7px 0",borderBottom:"1px solid #0c0a14",cursor:"pointer"}} onClick={function(){setDebateTicker(s.ticker);setDebateMode(true);}}>
            ...
            <span style={{fontFamily:"Orbitron",fontSize:"9px",padding:"2px 8px",background:sc(s.signal)+"22",color:sc(s.signal),border:"1px solid "+sc(s.signal)+"44",minWidth:"42px",textAlign:"center"}}>{s.signal}</span>
            <div style={{flex:1,height:"4px",background:"#0c0a14",borderRadius:"2px"}}><div style={{height:"100%",width:s.conf+"%",background:sc(s.signal),borderRadius:"2px"}}/></div>
            <span style={{fontFamily:"Orbitron",fontSize:"9px",color:sc(s.signal),minWidth:"32px",textAlign:"right"}}>{s.conf}%</span>
            <span style={{fontSize:"9px",color:parseFloat(s.chg)>0?CG:CR,minWidth:"50px",textAlign:"right"}}>{parseFloat(s.chg)>0?"+":""}{s.chg}%</span>
          </div>
        );})}
        <div style={{marginTop:"8px",fontSize:"9px",color:CD}}>Live momentum signals. Tap any ticker for Bull vs Bear debate.</div>
      </Panel>
      <Panel label={"â CONVICTION SUMMARY"}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px"}}>
          {[["BUY",buys,CG],["HOLD",holds,CY],["SELL",sells,CR]].map(function(row,ri){return(
...
function StockPopup({ticker,price,cost,shares,gainP,chgP,onClose}){
  var up=gainP>=0,tUp=chgP>=0,mktVal=parseFloat(price)*shares,costVal=parseFloat(cost)*shares,totalGain=mktVal-costVal;
  var rec=gainP>20?"HOLD â STRONG":gainP<-20?"REVIEW â THESIS CHECK":chgP<-5?"WATCH â SHARP DROP":chgP>5?"CONSIDER TRIM":"HOLD â NORMAL";
  var body=gainP>20?"Strong return. Let this winner run unless it exceeds 25% of portfolio.":gainP<-20?"Position underwater. Re-evaluate if original thesis still holds.":chgP<-5?"Sharp single-day drop. Check for news before acting.":chgP>5?"Strong day. Consider trimming to lock gains if overweight.":"Position within normal range. No action required.";
  return(
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(3,3,8,.93)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:BP,border:"2px solid "+CA+"55",padding:"20px",maxWidth:"460px",width:"92%",maxHeight:"82vh",overflowY:"auto"}} onClick={function(e){e.stopPropagation();}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"14px"}}>
          ...
          <button className="xbtn" onClick={onClose}>Ã</button>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"6px",marginBottom:"12px"}}>
          {[["PRICE","$"+parseFloat(price).toFixed(2),CB],["TODAY",(tUp?"+":"")+parseFloat(chgP).toFixed(2)+"%",tUp?CG:CR],["TOTAL",(up?"+":"")+parseFloat(gainP).toFixed(2)+"%",up?CG:CR]].map(function(row,ri){return(<div key={ri} style={{background:BD,padding:"10px",textAlign:"center",border:"1px solid #1a1520"}}><div style={{fontSize:"8px",color:CD,letterSpacing:"1px",marginBottom:"3px"}}>{row[0]}</div><div style={{fontFamily:"Orbitron",fontSize:"13px",fontWeight:700,color:row[2]}}>{row[1]}</div></div>);})}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px",marginBottom:"12px"}}>
          {[["SHARES",parseFloat(shares).toFixed(3)],["AVG COST","$"+parseFloat(cost).toFixed(2)],["MKT VALUE","$"+mktVal.toFixed(2)],["TOTAL P&L",(totalGain>=0?"+$":"-$")+Math.abs(totalGain).toFixed(2)]].map(function(row,ri){return(<div key={ri} style={{background:BD,padding:"8px",border:"1px solid #1a1520"}}><div style={{fontSize:"9px",color:CD}}>{row[0]}</div><div style={{fontFamily:"Orbitron",fontSize:"11px",color:CB,marginTop:"2px"}}>{row[1]}</div></div>);})}
        </div>
        <div style={{padding:"12px",background:"rgba(240,163,10,.06)",border:"1px solid "+CA+"33",marginBottom:"10px"}}>
          ...
          <div style={{fontSize:"10px",color:CC,lineHeight:1.8}}>{body}</div>
        </div>
        <div style={{fontSize:"9px",color:CD,lineHeight:1.6}}>At {parseFloat(shares).toFixed(3)} shares your exposure is ${mktVal.toFixed(2)}. No single name should exceed 20% of portfolio.</div>
      </div>
    </div>
  );
}
