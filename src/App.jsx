...
function generateEOD(enriched,totCost,totMkt,totGain,totP,bench,fg){
  ...
  var top=byDay[0],bot=byDay[byDay.length-1];
  var upCt=wd.filter(function(p){return p.chgP>0;}).length;
  var dnCt=wd.filter(function(p){return p.chgP<0;}).length;
  var dayDollar=wd.reduce(function(acc,p){return acc+(p.mkt&&p.chgP?(p.mkt-p.mkt/(1+p.chgP/100)):0);},0);
  ...
  out.push("");
  out.push("AI cluster: "+aiUp+" up, "+aiDn+" down.");
  if(bd.SPY) out.push("Market: SPY "+bd.SPY.chg+"%.");
  out.push("");
  out.push("Sentiment: "+(fg&&fg.label||"Neutral")+". Stay disciplined.");
  return out.filter(function(l,li){return li===0||l.length>0;}).join("\n");
}
...
.xbtn{background:transparent;color:#7a6030;border:1px solid #7a603044;padding:2px 10px;cursor:pointer;font-family:"Share Tech Mono",monospace;font-size:16px;}
  ...
  return (
    <div style={{height:"100vh",overflow:"hidden",background:BG,color:CC,fontFamily:"'Share Tech Mono',monospace",display:"flex",flexDirection:"column",position:"relative"}} className={alertOn?"alertMode":""}>
      ...
      <div style={{flex:1,minHeight:0,overflow:"auto",position:"relative",zIndex:1,padding:"12px 14px"}}>
        ...
        {tab==="alerts"    && <TabAlerts filings={edgarFilings} loading={edgarLoading} onScan={async function(){setEdgarLoading(true);var f=await fetchEdgarFilings();setEdgarFilings(f);setEdgarLoading(false);}}/>}
...
function TabBriefing({alf,onAlf,wx,city,setCity,onWx,fg,enriched,totP,eod,onEod}) {
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
      ...
      <Panel label={"Ã¢ÂÂ EOD BRIEFING"} right={<button className="btn bsm" onClick={onEod}>GENERATE</button>}>
        {!eod&&<div style={{fontSize:"10px",color:CD}}>Reads live prices Ã¢ÂÂ hit Refresh on Portfolio first, then GENERATE.</div>}
        {eod&&<div className="fi">{eod.split("\n").map(function(line,li){return li===0?<div key={li} style={{fontFamily:"Orbitron",fontSize:"9px",color:CA,marginBottom:"8px"}}>{line}</div>:line===""?<div key={li} style={{height:"4px"}}/>:<div key={li} style={{fontSize:"11px",color:CB,lineHeight:1.8}}>{line}</div>;})}<div style={{marginTop:"8px",fontSize:"9px",color:CD}}>Ã¢ÂÂ MARCUS</div></div>}
      </Panel>
      ...
      <Panel label={"Ã¢ÂÂ POSITIONS Ã¢ÂÂ "+POSITIONS.length+" HOLDINGS "+(live?"Ã¢ÂÂª LIVE":"Ã¢ÂÂª SESSION DATA 4/14")} right={<button className="btn bsm" onClick={onRefresh} disabled={status==="loading"}>Ã¢ÂÂ» REFRESH</button>}>
        ...
        {status!=="loading"&&enriched.map(function(p,i){return (
          <div key={p.ticker} className="pr si" style={{animationDelay:i*30+"ms"}}>
            ...
            <Panel label={"Ã¢ÂÂ TARGET CALCULATOR"}>
        ...
        <button className="btn" style={{width:"100%",marginBottom:"8px"}} onClick={function(){
          ...
          var need=g-cur;
          if(need<=0){setTgtResult("Already at goal! Current: $"+cur.toFixed(2));return;}
          var top=enriched.filter(function(px){return px.chgP!=null;}).slice().sort(function(a,bx){return (bx.chgP||0)-(a.chgP||0);}).slice(0,4);
          var msg="Need $"+need.toFixed(2)+" over "+d+" days. Buys: "+top.map(function(px){return px.ticker+" $"+(need*0.25/(parseFloat(px.price)||px.avgCost)).toFixed(2)+"sh";}).join(", ");
          setTgtResult(msg);
        }}>CALCULATE</button>
...
async function fetchEdgarFilings(){
  ...
  await Promise.all(Object.entries(CIKS).map(async function(kv){
    try{
      var r=await fetch("https://data.sec.gov/submissions/CIK"+kv[1]+".json",{headers:{"User-Agent":"apex/1.0 apex@app.com"}});
      var d=await r.json();
...
function TabAlerts({filings,loading,onScan}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      <Panel label={"Ã¢ÂÂ EDGAR SCANNER"}>
        <button className="btn" style={{width:"100%",marginBottom:"10px"}} onClick={onScan}>{loading?"SCANNING...":"Ã¢ÂÂ» SCAN SEC EDGAR Ã¢ÂÂ ALL POSITIONS"}</button>
      ...
      {urgent.length>0&&<Panel label={"Ã¢ÂÂ URGENT ("+urgent.length+")"}>{urgent.map(function(f,fi){return <div key={fi} style={{padding:"8px",marginBottom:"4px",background:"rgba(224,48,16,.06)",border:"1px solid "+CR+"44"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontFamily:"Orbitron",fontSize:"11px",color:CA}}>{f.ticker}</span><span style={{fontFamily:"Orbitron",fontSize:"8px",color:CR}}>{fl[f.form]||f.form}</span><span style={{fontSize:"9px",color:CD}}>{f.date}</span></div><a href={f.url} target="_blank" rel="noreferrer" style={{fontSize:"9px",color:CA,textDecoration:"none"}}>VIEW ON SEC.GOV</a></div>;})}</Panel>}
      <Panel label={"Ã¢ÂÂ FILINGS Ã¢ÂÂ LAST 7 DAYS"+(filings.length>0?" ("+filings.length+")":"")}>
        ...
        {!loading&&normal.map(function(f,fi){
          return <div key={fi} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:"1px solid #0c0a14"}}><div style={{display:"flex",gap:"8px",alignItems:"center"}}><span style={{fontFamily:"Orbitron",fontSize:"11px",color:CA}}>{f.ticker}</span><span style={{fontFamily:"Orbitron",fontSize:"8px",padding:"1px 5px",background:fc(f.form)+"22",color:fc(f.form)}}>{fl[f.form]||f.form}</span></div><div style={{display:"flex",gap:"8px"}}><span style={{fontSize:"9px",color:CD}}>{f.date}</span><a href={f.url} target="_blank" rel="noreferrer" style={{fontSize:"9px",color:CC,textDecoration:"none"}}>VIEW</a></div></div>;
        })}
      </Panel>
    </div>
  );
...
function TabMission({tasks,newTask,setNewTask,onAdd,onToggle,onDel,wl,newWl,setNewWl,onAddWl,onDelWl,journal,setJournal,enriched}) {
  ...
  var se=enriched||[];
  var [predLog,setPredLog]=useState(function(){try{var s=localStorage.getItem("apex-pred");return s?JSON.parse(s):[];}catch(e){return [];}});
  var [pTicker,setPTicker]=useState("");
  var [pCall,setPCall]=useState("BUY");
  var [pNote,setPNote]=useState("");
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
        ...
        <Panel label={"Ã¢ÂÂ PARLAYS SUMMARY"}>
          ...
          <div style={{marginTop:"8px",fontSize:"9px",color:CD,letterSpacing:"1px"}}>AT RISK: $112 Ã¢ÂÂª MAX PAYOUT: $13,008.93 Ã¢ÂÂª SETTLES OCT 31</div>
            ...
            <Panel label={"Ã¢ÂÂ PREDICTION LOG Ã¢ÂÂ SELF LEARNING"}>
        <div style={{display:"flex",gap:"6px",marginBottom:"6px"}}>
          <select style={{flex:1,background:BD,color:CB,border:"1px solid #2a1e08",padding:"6px 8px",fontFamily:"Share Tech Mono,monospace",fontSize:"11px"}} value={pTicker} onChange={function(e){setPTicker(e.target.value);}}><option value="">TICKER...</option>{POSITIONS.map(function(pos){return <option key={pos.ticker} value={pos.ticker}>{pos.ticker}</option>;})}</select>
          <select style={{width:"80px",background:BD,color:CB,border:"1px solid #2a1e08",padding:"6px 8px",fontFamily:"Share Tech Mono,monospace",fontSize:"11px"}} value={pCall} onChange={function(e){setPCall(e.target.value);}}><option>BUY</option><option>SELL</option><option>HOLD</option></select>
        </div>
        <input className="inp" style={{width:"100%",marginBottom:"6px"}} value={pNote} onChange={function(e){setPNote(e.target.value);}} placeholder="REASONING..."/>
        <button className="btn" style={{width:"100%",marginBottom:"10px"}} onClick={function(){
          if(!pTicker) return;
          var pos=se.find(function(p){return p.ticker===pTicker;});
          var entry={id:Date.now(),date:new Date().toISOString().slice(0,10),ticker:pTicker,call:pCall,price:pos?parseFloat(pos.price||pos.avgCost):0,note:pNote,outcome:"OPEN"};
          var u=[entry].concat(predLog);
          setPredLog(u);localStorage.setItem("apex-pred",JSON.stringify(u));
          setPTicker("");setPNote("");
        }}>LOG PREDICTION</button>
        {predLog.length===0&&<div style={{fontSize:"10px",color:CD}}>No predictions yet.</div>}
        {predLog.slice(0,8).map(function(pred,pi){
          var oc=pred.outcome==="WIN"?CG:pred.outcome==="LOSS"?CR:CY;
          var pos2=se.find(function(p){return p.ticker===pred.ticker;});
          var cur2=pos2?parseFloat(pos2.price||pos2.avgCost):null;
          var pnl=cur2&&pred.price?((cur2-pred.price)/pred.price*100).toFixed(1):null;
          return(
            <div key={pred.id} style={{padding:"8px",marginBottom:"4px",background:BD,border:"1px solid #1a1520"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div style={{display:"flex",gap:"8px"}}>
                  <span style={{fontFamily:"Orbitron",fontSize:"11px",color:CA}}>{pred.ticker}</span>
                  <span style={{fontFamily:"Orbitron",fontSize:"8px",padding:"1px 5px",background:(pred.call==="BUY"?CG:pred.call==="SELL"?CR:CY)+"22",color:pred.call==="BUY"?CG:pred.call==="SELL"?CR:CY}}>{pred.call}</span>
                  <span style={{fontFamily:"Orbitron",fontSize:"8px",padding:"1px 5px",background:oc+"22",color:oc}}>{pred.outcome}</span>
                </div>
                <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
                  {pnl&&<span style={{fontSize:"10px",color:parseFloat(pnl)>0?CG:CR}}>{parseFloat(pnl)>0?"+":""}{pnl}%</span>}
                  <button style={{background:"transparent",border:"none",color:CD,cursor:"pointer",fontSize:"12px"}} onClick={function(){var u=predLog.map(function(p){return p.id===pred.id?Object.assign({},p,{outcome:p.outcome==="WIN"?"LOSS":p.outcome==="LOSS"?"OPEN":"WIN"}):p;});setPredLog(u);localStorage.setItem("apex-pred",JSON.stringify(u));}}>o</button>
                  <button style={{background:"transparent",border:"none",color:CD,cursor:"pointer",fontSize:"12px"}} onClick={function(){var u=predLog.filter(function(p){return p.id!==pred.id;});setPredLog(u);localStorage.setItem("apex-pred",JSON.stringify(u));}}>x</button>
                </div>
              </div>
              <div style={{fontSize:"9px",color:CD,marginTop:"3px"}}>${pred.price.toFixed(2)} {pred.note?"Ã¢ÂÂ "+pred.note.slice(0,40):""} {pred.date}</div>
            </div>
          );
        })}
      </Panel>
      ...
      {view==="calc"&&(
        <Panel label={"Ã¢ÂÂ CASH OUT CALCULATOR Ã¢ÂÂ ALL 4 PARLAYS"}>
          ...
          <div style={{padding:"10px",background:"#08080f",border:"1px solid "+CA+"33"}}>
            ...
            <Panel label={"◈ MLB HOT STREAK — O/U SIGNAL"}>
        <div style={{fontSize:"9px",color:CD,lineHeight:1.7,marginBottom:"10px"}}>Teams hot in last 10 games (7+ wins) lean OVER. Cold teams (3 or fewer wins) lean UNDER.</div>
        {standing&&standing.length>0?(
          <div>
            {standing.filter(function(t){return t.l10w>=7||t.l10w<=3;}).map(function(t,ti){
              var hot=t.l10w>=7,col=hot?CG:CR,signal=hot?"LEAN OVER":"LEAN UNDER";
              var conf=hot?(t.l10w-5)*15:(5-t.l10w)*15;
              return(
                <div key={ti} style={{display:"flex",alignItems:"center",gap:"8px",padding:"6px 0",borderBottom:"1px solid #0c0a14"}}>
                  <span style={{fontFamily:"Orbitron",fontSize:"11px",fontWeight:800,color:CA,minWidth:"38px"}}>{t.t}</span>
                  <span style={{fontFamily:"Orbitron",fontSize:"8px",padding:"1px 6px",background:col+"22",color:col,border:"1px solid "+col+"44"}}>{hot?"HOT":"COLD"}</span>
                  <span style={{fontFamily:"Orbitron",fontSize:"8px",color:col,minWidth:"88px"}}>{signal}</span>
                  <div style={{flex:1,height:"3px",background:"#0c0a14",borderRadius:"2px"}}>
                    <div style={{height:"100%",width:Math.min(100,conf)+"%",background:col,borderRadius:"2px"}}></div>
                  </div>
                  <span style={{fontSize:"9px",color:CD}}>L10: {t.l10w}-{t.l10l}</span>
                </div>
              );
            })}
            <div style={{marginTop:"8px",fontSize:"9px",color:CD}}>Hot=7+ wins. Cold=3 or fewer. One signal among many.</div>
          </div>
        ):<div style={{fontSize:"10px",color:CD}}>Load standings first.</div>}
      </Panel>
      )}
    </div>
  );
...
function StockPopup({ticker,price,cost,shares,gainP,chgP,onClose}){
  var up=gainP>=0,tUp=chgP>=0,mv=parseFloat(price)*shares,cv=parseFloat(cost)*shares,tg=mv-cv;
  var rec=gainP>20?"HOLD":gainP<-20?"REVIEW":chgP<-5?"WATCH":chgP>5?"TRIM":"HOLD";
