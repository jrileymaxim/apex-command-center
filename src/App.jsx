...
function calcMACD(prices){
  ...
  function ema(pts,n){
    var k=2/(n+1),e=pts[0];
    for(var i=1;i<pts.length;i++) e=pts[i]*k+e*(1-k);
    return e;
  }
  var ema12=ema(prices,12),ema26=ema(prices,26);
  var macd=ema12-ema26;
  return {macd:macd.toFixed(3),signal:(macd>0?"BULLISH":"BEARISH"),ema12:ema12.toFixed(2),ema26:ema26.toFixed(2)};
}

function technicalSignal(rsi,macd,chgP){
  var score=0;
  if(rsi!=null){
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      ...
      <Panel label={"â POSITIONS â "+POSITIONS.length+" HOLDINGS "+(live?"âª LIVE":"âª SESSION DATA 4/14")} right={<button className="btn bsm" onClick={onRefresh} disabled={status==="loading"}>â» REFRESH</button>}>
        ...
        <button className="btn" style={{width:"100%",marginBottom:"8px"}} onClick={function(){
          ...
          var msg="Need $"+need.toFixed(2)+" over "+d+" days. Top buys: "+top.map(function(px){return px.ticker+" "+( need*0.25/(parseFloat(px.price)||px.avgCost)).toFixed(2)+"sh";}).join(", ");
          setTgtResult(msg);
        }}>CALCULATE</button>
...
function TabIntel({enriched,status,totP,fg}){
  ...
  // Build signals using live price data + technical indicators
  var signals=enriched.map(function(p){
    var chg=parseFloat(p.chgP)||0,gain=p.gainP||0;
    // Simple RSI approximation from gain/chg data
    var rsiApprox=chg>3?72:chg>1?58:chg<-3?28:chg<-1?42:50;
    var signal=technicalSignal(rsiApprox,{signal:chg>0?"BULLISH":"BEARISH"},chg);
    var conf=Math.min(92,Math.max(25,Math.abs(chg)*14+Math.abs(gain)*0.4+42));
    return {ticker:p.ticker,signal:signal,conf:conf.toFixed(0),chg:chg.toFixed(2),gain:gain.toFixed(1),price:(parseFloat(p.price)||p.avgCost).toFixed(2),rsi:rsiApprox};
  });
        ...
        {status!=="loading"&&signals.map(function(s,si){
          ...
          return(
            <div key={si} style={{display:"flex",alignItems:"center",gap:"8px",padding:"7px 0",borderBottom:"1px solid #0c0a14",cursor:"pointer"}} onClick={function(){setDebateTicker(s.ticker);setDebateMode(true);}}>
              ...
              <span style={{fontFamily:"Orbitron",fontSize:"9px",color:sc(s.signal),minWidth:"30px",textAlign:"right"}}>{s.conf}%</span>
              <span style={{fontSize:"9px",color:rsiColor,minWidth:"48px",textAlign:"right"}}>RSI:{s.rsi}</span>
...
function StockChart({ticker,price,chg}){
  ...
  // Fetch 30-day price history from Yahoo Finance via our API
  useState(function(){
    setLoading(true);
    fetch("/api/prices?symbols="+ticker+"&range=30d")
      .then(function(r){return r.json();})
      .then(function(d){
        var results=(d.quoteResponse&&d.quoteResponse.result)||[];
        if(results.length>0&&results[0].history){
          setHistory(results[0].history);
        } else {
          // Generate synthetic sparkline from current price for demo
          var pts=[];
          var base=parseFloat(price||100);
          var chgPct=parseFloat(chg||0)/100;
          for(var i=29;i>=0;i--){
            var noise=(Math.random()-0.5)*0.02;
            var trend=chgPct/30*(29-i);
            pts.push({time:Math.floor(Date.now()/1000)-i*86400, value:parseFloat((base*(1-chgPct+trend+noise)).toFixed(2))});
          }
          setHistory(pts);
        }
        setLoading(false);
  ...
  var up=parseFloat(chg||0)>=0;
  var lineColor=up?"#18c93a":"#e03010";
  var areaTopColor=up?"rgba(24,201,58,.25)":"rgba(224,48,16,.25)";
  var areaBottomColor=up?"rgba(24,201,58,.02)":"rgba(224,48,16,.02)";

  return(
    <div style={{marginTop:"8px"}}>
      {loading&&<div style={{fontSize:"9px",color:"#7a6030",padding:"8px 0"}}>Loading chart...</div>}
          ...
          <svg width="100%" height="60" viewBox={"0 0 300 60"} preserveAspectRatio="none">
            {(function(){
              ...
              var pts=vals.map(function(v,idx){
                var x=(idx/(vals.length-1))*300;
                var y=55-((v-min)/range)*50;
                return x.toFixed(1)+","+y.toFixed(1);
              });
              var linePath="M "+pts.join(" L ");
              var areaPath=linePath+" L 300,58 L 0,58 Z";
              return[
                <path key="a" d={areaPath} fill={areaTopColor} stroke="none"/>,
                <path key="l" d={linePath} fill="none" stroke={lineColor} strokeWidth="1.5"/>
              ];
            })()}
          </svg>
          <div style={{position:"absolute",bottom:"2px",right:"4px",fontSize:"8px",color:up?"#18c93a":"#e03010",fontFamily:"Orbitron"}}>{parseFloat(chg||0)>0?"+":""}{parseFloat(chg||0).toFixed(2)}%</div>
        </div>
      )}
    </div>
  );
    ...
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(3,3,8,.93)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:BP,border:"2px solid "+CA+"55",padding:"20px",maxWidth:"460px",width:"92%",maxHeight:"82vh",overflowY:"auto"}} onClick={function(e){e.stopPropagation();}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"14px"}}>
          ...
          <button style={{background:"transparent",border:"none",color:CD,cursor:"pointer",fontSize:"20px",lineHeight:1}} onClick={onClose}>x</button>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px",marginBottom:"10px",marginTop:"10px"}}>
