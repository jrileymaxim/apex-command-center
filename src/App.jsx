  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
      ...
      <Panel label={"â EOD BRIEFING"} right={<button className="btn bsm" onClick={onEod}>GENERATE</button>}>
        {!eod&&<div style={{fontSize:"10px",color:CD}}>Reads live prices â hit Refresh on Portfolio first, then GENERATE.</div>}
        {eod&&<div className="fi">{eod.split("\n").map(function(line,li){return li===0?<div key={li} style={{fontFamily:"Orbitron",fontSize:"9px",color:CA,marginBottom:"8px"}}>{line}</div>:line===""?<div key={li} style={{height:"4px"}}/>:<div key={li} style={{fontSize:"11px",color:CB,lineHeight:1.8}}>{line}</div>;})}<div style={{marginTop:"8px",fontSize:"9px",color:CD}}>â MARCUS</div></div>}
      </Panel>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
        ...
        <Panel label={"â PARLAYS SUMMARY"}>
          ...
          <div style={{marginTop:"8px",fontSize:"9px",color:CD,letterSpacing:"1px"}}>AT RISK: $112 âª MAX PAYOUT: $13,008.93 âª SETTLES OCT 31</div>
            ...
            <Panel label={"â PREDICTION LOG â SELF LEARNING"}>
        ...
        {predLog.slice(0,8).map(function(pred,pi){
          ...
          return(
            <div key={pred.id} style={{padding:"8px",marginBottom:"4px",background:BD,border:"1px solid #1a1520"}}>
              ...
              <div style={{fontSize:"9px",color:CD,marginTop:"3px"}}>${pred.price.toFixed(2)} {pred.note?"â "+pred.note.slice(0,40):""} {pred.date}</div>
            </div>
          );
        })}
      </Panel>
...
function TabParlays(){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
      ...
      <Panel label={"◈ MLB HOT STREAK MODEL — O/U SIGNAL"}>
        <div style={{fontSize:"9px",color:CD,lineHeight:1.7,marginBottom:"10px"}}>Pattern: teams hot in last 3 games (2+ wins) tend to cover the over. Cold teams (2+ losses) lean under. Strength multiplied by run differential and Vegas O/U line.</div>
        {standing&&standing.length>0?(
          <div>
            {standing.filter(function(t){return t.last10>0;}).map(function(t,ti){
              var last3=t.l10w>=7?"HOT":t.l10w<=3?"COLD":"NEUTRAL";
              if(last3==="NEUTRAL") return null;
              var signal=last3==="HOT"?"LEAN OVER":"LEAN UNDER";
              var col=last3==="HOT"?CG:CR;
              var conf=last3==="HOT"?(t.l10w-5)*15:(5-t.l10w)*15;
              return(
                <div key={ti} style={{display:"flex",alignItems:"center",gap:"8px",padding:"6px 0",borderBottom:"1px solid #0c0a14"}}>
                  ...
                  <span style={{fontFamily:"Orbitron",fontSize:"8px",padding:"1px 6px",background:col+"22",color:col,border:"1px solid "+col+"44"}}>{last3}</span>
                  <span style={{fontFamily:"Orbitron",fontSize:"8px",color:col,minWidth:"90px"}}>{signal}</span>
                  <div style={{flex:1,height:"3px",background:"#0c0a14"}}><div style={{height:"100%",width:Math.min(100,conf)+"%",background:col}}/></div>
                  <span style={{fontSize:"9px",color:CD}}>L10: {t.l10w}-{t.l10l}</span>
                </div>
              );
            }).filter(Boolean)}
            <div style={{marginTop:"8px",fontSize:"9px",color:CD}}>Hot = 7+ wins in L10. Cold = 3 or fewer wins. Signal confidence scales with streak strength. Not financial advice — use as one input among many.</div>
          </div>
        ):<div style={{fontSize:"10px",color:CD}}>Load Parlays tab standings to see hot streak signals.</div>}
      </Panel>
      )}
    </div>
  );
