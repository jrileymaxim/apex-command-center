...
export default function App() {
  ...
  useEffect(function(){
    (async function(){
      try {
        var tr = await window.storage.get("apex-tasks");
        if (tr&&tr.value) setTasks(JSON.parse(tr.value));
        else setTasks([
        ...
        var wr2 = await window.storage.get("apex-wl");
        if (wr2&&wr2.value) setWl(JSON.parse(wr2.value));
      } catch(e) {
  ...
  useEffect(function(){
    if(!tlReady)return;
    window.storage.set("apex-tasks",JSON.stringify(tasks)).catch(function(){});
  },[tasks,tlReady]);

  useEffect(function(){
    if(!tlReady)return;
    window.storage.set("apex-wl",JSON.stringify(wl)).catch(function(){});
  },[wl,tlReady]);
