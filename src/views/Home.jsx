...
const RH_ACTUALS = {
  AAL:  { gainAmt: 17.24,  gainPct: 12.01 },
  MNTS: { gainAmt: 6.17,   gainPct: 2.80  },
  ANET: { gainAmt: 42.43,  gainPct: 14.81 },
  TSM:  { gainAmt: -20.29, gainPct: -0.80 },
  MU:   { gainAmt: 39.68,  gainPct: 9.52  },
  NVDA: { gainAmt: 48.57,  gainPct: 5.42  },
  SOUN: { gainAmt: -0.23,  gainPct: -0.94 },
  VTI:  { gainAmt: 36.16,  gainPct: 3.56  },
  CRWV: { gainAmt: 55.89,  gainPct: 4.58  },
  GLD:  { gainAmt: 1.86,   gainPct: 1.06  },
  BBAI: { gainAmt: 17.50,  gainPct: 9.83  },
  XYL:  { gainAmt: -0.01,  gainPct: -0.84 },
  PNR:  { gainAmt: -0.72,  gainPct: -0.78 },
  AWK:  { gainAmt: 1.11,   gainPct: 0.85  },
  VLTO: { gainAmt: -0.36,  gainPct: -0.77 },
  GWRS: { gainAmt: 0.20,   gainPct: 0.26  },
  CDZI: { gainAmt: -0.36,  gainPct: -0.82 },
  GRC:  { gainAmt: -0.05,  gainPct: -1.67 },
};

function getSignal(dayPct, analystWithPrice, insider) {
  const upside = analystWithPrice?.mean && analystWithPrice?.price
    ? ((analystWithPrice.mean - analystWithPrice.price) / analystWithPrice.price * 100)
    : null;
  if (insider?.sold && !insider?.bought) return { label: 'SELL', color: R };
  if (upside !== null && upside > 20 && (dayPct || 0) > 0 && insider?.bought) return { label: 'STRONG BUY', color: G };
...
function TopPickCard() {
  ...
  if (loading) return (
    <div style={{ background: 'rgba(255,215,0,0.04)', border: '1px solid rgba(255,215,0,0.2)', borderRadius: 8, padding: '12px 16px', marginBottom: 12 }}>
      <div style={{ fontSize: 9, color: Y, letterSpacing: 2, marginBottom: 6 }}>AI TOP PICK — SCANNING MARKET...</div>
  ...
  return (
    <div>
      <div
        ...
        style={{ display: 'grid', gridTemplateColumns: '50px 1fr 60px 65px 75px 80px 90px 30px', gap: 6, alignItems: 'center', padding: '8px 12px', background: isAlert ? 'rgba(255,215,0,0.04)' : 'transparent', borderBottom: expanded ? 'none' : '1px solid rgba(255,255,255,0.05)', borderLeft: isAlert ? '2px solid ' + Y : '2px solid transparent', cursor: 'pointer' }}
      >
...
export default function Home() {
  ...
  const portfolioValue = sync
    ? sync.portfolioValue
    : (equityPositions.reduce((a, p) => { const px = quotes[p.ticker]?.price ?? p.avgCost; return a + px * p.shares; }, 0) + leapsMkt);
  const totalReturn = sync
    ? sync.totalReturn
    : (Object.values(RH_ACTUALS).reduce((a, v) => a + v.gainAmt, 0) + leapsPnl);
  ...
  const dayPnl = equityPositions.reduce((a, p) => {
    const q = quotes[p.ticker];
    if (!q?.price || !q?.chgPct) return a;
    const prev = q.price / (1 + q.chgPct / 100);
    return a + (q.price - prev) * p.shares;
  }, 0);

  const alerts = equityPositions.filter(p => {
    const c = quotes[p.ticker]?.chgPct;
    return c !== undefined && Math.abs(c) >= 3;
  }).length;
  ...
  const grouped = THEME_ORDER.map(tid => ({
    ...
    positions: equityPositions
      .filter(p => p.themeIds?.includes(tid))
      .sort((a, b) => Math.abs(quotes[b.ticker]?.chgPct || 0) - Math.abs(quotes[a.ticker]?.chgPct || 0)),
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#080810', color: '#fff', fontFamily: 'JetBrains Mono,monospace', padding: '0 0 40px' }}>
      ...
      <div style={{ maxWidth: 980, margin: '0 auto', padding: '16px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '50px 1fr 60px 65px 75px 80px 90px 30px', gap: 6, padding: '6px 12px', marginBottom: 4 }}>
          {['TICKER', 'THEME', 'PRICE', 'DAY', 'P+L', 'RETURN', 'SIGNAL', ''].map(h => (
            <span key={h} style={{ fontSize: 9, color: DIM, letterSpacing: 1 }}>{h === 'P+L' ? 'P&amp;L' : h}</span>
          ))}
        </div>
